...
**Status: designed, DEFERRED — do not start speculatively.** A separate epic that follows the tile-pipeline
unification (`docs/per-layer-tile-processing-design.md`, complete through A7). **Natural trigger: the GeoJSON
payload (A8)** — a real non-command-stream format proves the seam rather than another synthetic fixture. Bank
the design; execute on demand. The verdict — **defer the whole epic, including B1, until a second format
actually pulls it** — is at the end (§Should we build this yet?).
...
4. **Everything is tiled.** GeoJSON is sliced client-side (geojson-vt), and geojson-vt emits extent-scaled
   **tile-local** integer coords, exactly like MVT. There is no non-tiled format in the system; tile-local *is*
   the vector-neutral shape every format slices to. This makes geodetic the wrong altitude for the *interchange
...
call** — the mesh and symbol worker passes run sequentially inside one kick task, so a single `using`-scoped
buffer suffices (no refcount, no cross-thread handoff). This is a **genuinely new** ownership question — native
...
- **(B) Remove `uint[]` from `ITileFeature` → YES, entirely (not null), last, gated twice.** Make the interface a
  pure evaluation surface (`Id`/`Properties`/`GeometryType`, all already on `IFeature`); geometry travels only in
  the blittable buffer, joined by ordinal. `GeometryType` **stays** (selection needs it; also load-bearing for
  ring-kind per landmine #1). Fire B5 on **(a)** a grep proving zero production `.Geometry` readers after B3/B4,
  **AND (b)** A8/GeoJSON actually landing. **In-tree evidence:** `TileBackgroundLayerProcessor.cs:37`
