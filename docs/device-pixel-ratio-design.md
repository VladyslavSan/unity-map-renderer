...
| 2 | ~~**No tooth pins the frozen-`_haloZoom` fence.**~~ **CLOSED (UMR-147, T5).** `_haloZoom` is deleted — the halo now routes through `ZoomStyleApplier` and tracks the live zoom, so the fence this finding wanted no longer has a mechanism to break. `SymbolHaloWidth_TracksTheLiveZoom` pins the live-zoom behaviour directly. | — |
