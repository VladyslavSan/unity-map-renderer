...
`DeviceScaling.LogicalToDevicePx` + `PixelSpace` (Unity, engine-free) → `ZoomStyleApplier`'s two device-pixel
...
| T3-6 | *structural.* The ratio fallback exists in exactly **one** production file (`Unity/View/DeviceScaling.cs`) — D2 made checkable rather than hoped-for. Asserts the positive match too, so a broken predicate that matches nothing still fails |
...
* ~~**A zoom-expression halo** (design §7 risk 9) is still a follow-up.~~ **CLOSED (UMR-147).** The halo now
  routes through a `ZoomStyleApplier` (like fill/line) and re-evaluates at the **live** zoom every frame.
...
|---|---|---|
| 1 | ~~**The two new binding loops have no zero-alloc tooth.**~~ **PARTIALLY CLOSED (UMR-147, T8b).** The device-pixel **float** loop is now exercised (zero-alloc) by two `ZoomStyleApplierTests` methods. The **vector** half was independently closed at `8e21c14d` by `StyleTransitionBindingTests.MixedApplier`, in a different fixture from the one this row points at. | Deferred only to keep the gate-verified tree frozen at commit; discharged here. |
| 2 | ~~**No tooth pins the frozen-`_haloZoom` fence.**~~ **CLOSED (UMR-147, T5), then overtaken.** `_haloZoom` is deleted, so the fence this finding wanted no longer has a mechanism to break. T5's live-zoom tooth went with the uniform when the halo became geometry: `text-halo-width` is now evaluated per feature at tile build and scaled to device px at emit, so it moves on a tile rebuild rather than every frame, and `SymbolHaloEmitTests` reads it off the emitted mesh. | — |
