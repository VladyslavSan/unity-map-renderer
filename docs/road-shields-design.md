...
The design derives from the public MapLibre Style Spec, this repo's source, and an empirical probe over a
committed fixture.
...
From `Assets/StreamingAssets/Fixtures/liberty.json`. All three share `source-layer:
...
a `concat` over `["get","network"]` rather than a literal prefix. Both differences matter to D1.

Facts the design relies on:

- Liberty uses **pre-sized** sprites `road_1..road_6` keyed by `ref_length`, so the sprite already fits its
  number, and this style never needs `icon-text-fit`.
- All 18 shield sprites exist in the live openfreemap sheet.
- `Value.FormatNumber` uses `"R"`, so `5` → `"5"`, and the resolved icon names hit the atlas.

---

## 3. Decisions

### D1 — `symbol-placement` is a **build-zoom**-evaluated `StyleProperty<SymbolPlacement>`

`LayoutProperties.SymbolPlacement` is a `StyleProperty<SymbolPlacement>`, not a bare enum, matching the
file's own convention that a zoom-capable property is `StyleProperty<T>`. A plain-string read would degrade
every expression-valued (`step`) placement to `Point`. The projector is
...
real fix needs display-zoom re-evaluation of `symbol-placement` ("Deferred / open").

### D2 — point placement on a LineString anchors at **mid-arc**
...
That gap is a choice ("Deferred / open").
...
### D4 — line placement + viewport-resolved alignment ⇒ **upright labels at the line anchors**
...
position, instead of one curved label per path. This is the road-shield look: `viewport` rotation-alignment
keeps each symbol aligned to the viewport, so it stays upright. Line placement therefore neither always
curves nor fences icons off.
...
`docs/labels-and-symbols-design.md` § "Map-aligned line icons + `icon-rotate` (P-B)". The viewport-resolved
case above is a separate path.

### D6 — a symbol build **waits for the sprite fetch to settle**; it never commits icon-starved
...
D1–D4 are pinned by `SymbolShieldExtractionTests` (in `Style/StyleShieldTests.cs`: placement-per-zoom,
the icon+text pair shape, the two US-only shield layers selecting zero Berlin-fixture features). D6 is
pinned by `SymbolSpriteReadinessTests` (in `Text/SymbolReconcileAsyncTests.cs`: the sprite-fetch gate,
...
| **The block's top/bottom edge definition** (text centring, "Text vertical centring") | Open — see "Why `Top`/`Bottom` keep the box edges" under D12. Measured but unconfirmed: a `top`-anchored label's ink starts about 9 baked px (0.375 em) below its anchor, so `poi_r*` / `airport` text plausibly sits lower than MapLibre's by roughly that much. Wants a maintainer eyeball before it becomes a decision. |
| **Deriving `GlyphSdf.BaselineBelowReferencePx` per font stack** instead of a constant (D12) | The glyph-PBF format carries no font-level metrics, so every consumer must assume one; 26 is measured from the committed fixture. A font baked against a different ascent needs the glyph-atlas view to expose a per-stack metric, plus a policy for stacks with no baseline-resting reference glyph (CJK-only labels). |
| **Re-tuning `GlyphSdf.NominalCapHeightEm` per script** (D12) | `17/24` is measured from Latin Noto Sans; no reported symptom for other scripts. |
| **`text-variable-anchor`, `text-writing-mode`, vertical CJK** | Not affected by this design. |
...
## 9. Render queue: a layer owns a **band** of sub-slots

A symbol layer's icon quad and its text quad are coplanar at the same anchor (a centred pair, "Symbol
pairing"), both
...
`LayerDrawOrder` is the one render-queue formula (its stated role), with a named sub-slot inside each
...
- Layer *i* owns the contiguous band `[base + i·S, base + i·S + S−1]`. Every sub-slot of layer *i* is
  strictly below every sub-slot of layer *i+1*, so the cross-layer draw-order guarantee holds for every
  sub-slot.
- **A symbol layer's icon is `Base`, its text is `Above`.** The badge draws under the number it frames.
- Fill / line / background use `Base` only, so their relative order is the declaration order (a strictly
...
`ScriptableRenderPass` draw-order target (`ARCHITECTURE.md` § "Layer ordering & draw submission"). A
stride of 2 gives that interim a runway of ~1000 layers, half of what a stride of 1 would — liberty uses
111.
...
## 10. Symbol pairing: a centred icon+text symbol is ONE placement instance

The Style Spec treats a symbol's icon and text as **one instance**: `icon-optional` / `text-optional`
default false, so they place or drop together, with a combined box. Two independent candidates would let
the icon lose while its number kept drawing. This renderer models the instance as one `SymbolCandidate`
spanning both halves' boxes.

**Scope note.** The predicate for *which* icon+text symbols pair this way is not shield-specific — it
covers non-centred pairs too, and its statement is owned by `docs/labels-and-symbols-design.md`
§ "Non-centred icon+text pairing (P-A)", which wins if this section and that one ever disagree. What
follows is the mechanism a paired shield symbol relies on.
...
candidate inserts them all — the spec's combined-box semantics, and the same machinery curved
...
if every candidate carrying it survived). This cannot work: the two boxes overlap, so the icon places,
inserts its box, and the text then collides with its own partner — every shield would drop.
...
emits adjacently. It does not force the text half's overlap flags: a pair's text carries its own authored
`text-allow-overlap`/`text-ignore-placement`, and the pair candidate's flags are the AND of the two
halves'.
...
through the pair as `PairOptional` on each half. Each half's optionality goes into
`SymbolCandidate.OptionalBoxMask` (bit 0 = owner/icon, bit 1 = rider/text), so the all-or-nothing
collision test gains per-box granularity: an optional box can fail to place while its non-optional partner
still does, with no second pass and no widened key.

**Rejected: gating pair *formation* on both flags being false.** Its argument — a centred pair's boxes
overlap, so un-pairing makes the halves mutually exclusive — does not hold once pairing covers
non-centred, disjoint boxes too (see the scope note above). `text-optional` also names an escape hatch on
the *instance*, which presupposes the pair still forms, rather than a condition on whether it forms at all.

No liberty **shield** layer sets either property, so this is inert for shields. The general mechanism is
owned by `docs/labels-and-symbols-design.md` § "Non-centred icon+text pairing (P-A)", D-PA-3.
...
## 11. Text vertical centring: `Center` positions the block's optical centre, not its box midpoint

A centred text block's box is not filled symmetrically by its glyphs, so centring on the box puts a
...
its ink, so centring it is exact. Box-centred text and ink-centred icons therefore disagree.

Box centring also makes a single-line centred label's vertical position depend on `text-line-height`,
even though line-height is a line-*stacking* property with nothing to stack at one line.

### D12 — `Center` positions the block's optical centre; `Top`/`Bottom` keep the box edges

Vertical anchoring has three cases, and only the centre case departs from the box:
...
the em conversion cancels, so `OpticalCentreBelowReferencePx` is `17.5`, not an approximation. A centred
multi-line block's optical centre is the midpoint between the first line's and the last line's, which is
why the multi-line term is `(lineCount - 1)/2` line-heights rather than `lineCount/2` — line spacing is
untouched, and a single-line label's position does not depend on line height.
...
- *Rejected: the ink bounds of the glyphs actually present.* Unstable — a cap/digit run, an x-height run
  ...
  text changed.
- *Rejected: the em box above the baseline.* Barely differs from box centring — caps occupy roughly the
  lower 70% of the em box, which is why a box-centred number reads low.
...
**Why `Top`/`Bottom` keep the box edges.** The same root makes the block's top and bottom *edges* suspect
too — a `text-anchor: top` label's ink starts below the anchor by the same slack. But the centring
requirement pins only the *difference* between ascent and descent (it must equal the cap height); the
absolute values of ascent and descent are constrained by nothing measured and by no reported symptom.
Redefining the edges means guessing two numbers and moving every top/bottom-anchored label in every style
on that guess — an open question, not a decision ("Deferred / open").

**Consequence.** `Center` is not the arithmetic midpoint of `Top` and `Bottom` — the layout box's top
