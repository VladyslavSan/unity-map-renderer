...
namespace MapRenderer.Tests.Structure
{
    ...
    public class TileProcessingStructureTests
    {
        ...
        public void GeometryTypeEnum_RenameIsComplete_NoOldTokenSurvives()
        {
            ...
            Assert.IsTrue(Directory.Exists(codeRoot), $"expected directory to exist at {codeRoot}");
            string testsRoot = Path.Combine(Application.dataPath, "Tests");
            Assert.IsTrue(Directory.Exists(testsRoot), $"expected directory to exist at {testsRoot}");
            ...
            Assert.IsEmpty(offenders,
                ...
                $"— Epic A / A6.1 renamed it to Core.Tiles.TileGeometryType; found the old name in: " +
