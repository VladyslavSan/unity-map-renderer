...
namespace MapRenderer.Tests.Json
{
    ...
    public class JsonCanonicalTests
    {
        ...
        [Test]
        public void CacheKey_IsKeyOrderInvariant()
        {
            Assert.AreEqual(
                JsonCanonical.CacheKey("style", Obj(("a", Num(1)), ("b", Num(2))), "extra"),
                JsonCanonical.CacheKey("style", Obj(("b", Num(2)), ("a", Num(1))), "extra"),
        ...
        [Test]
        public void CacheKey_IsSensitiveToContent()
        {
            Assert.AreNotEqual(
                JsonCanonical.CacheKey("style", Obj(("a", Num(1))), "extra"),
                JsonCanonical.CacheKey("style", Obj(("a", Num(2))), "extra"),
        ...
        [Test]
        public void CacheKey_ExtraComponent_SeparatesEqualContent()
        {
            JsonValue value = Obj(("a", Num(1)));
            Assert.AreNotEqual(
                JsonCanonical.CacheKey("style", value, "numbering=1"),
                JsonCanonical.CacheKey("style", value, "numbering=2"),
        ...
        /// the same prefix — see <see cref="JsonCanonical.CacheKey"/>'s own doc — so it must fail loud rather
        /// than silently mint a shared token.</summary>
        [Test]
        public void CacheKey_NullValue_Throws()
        {
            Assert.Throws<System.ArgumentNullException>(() => JsonCanonical.CacheKey("style", null, "extra"));
        }
    }
}
