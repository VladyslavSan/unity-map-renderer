...
// FillMeshBuildBuffersPoolTests) — it cannot discriminate a multi-megabyte allocation here. GC.GetTotalMemory
...
// before the pipeline tooth trusts it. NOT registered in core-tests.csproj (FillMeshGraph/NativeArray live
...
//
// job-scheduling-design.md §8 stage 4 Group B: FillMeshPipeline.Schedule (the synchronous entry point this
// tooth used to drive) is retired — the graph (FillMeshGraph.Schedule + Handle.Complete()) is the only mesher
// left, so this measures schedule-time managed allocation over IT instead. Same ceiling, same fixture, same
// calibrated meter — only the entry point changed.
