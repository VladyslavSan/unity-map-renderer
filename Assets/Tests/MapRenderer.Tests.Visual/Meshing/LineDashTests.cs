...
namespace MapRenderer.Tests.Visual
{
    ...
    [TestFixture]
    public class LineDashSnapshotTests : VisualTestFixture
    {
        ...
        private static float3 GlobalPlateau(Frame frame, float3 background)
        {
        ...
        private static float CoverageAtPixel(
            ...
            => PixelCoverage.CoverageAt(frame, column, row, background, plateau);
        ...
        private static float[] CoverageAlongRow(
            Frame pixels, int row, int columnFrom, int columnTo, float3 background, float3 plateau)
        {
        ...
        private static float MeasureBandThicknessPx(
            Frame pixels, int centreRow, int columnFrom, int columnTo, float3 background, float3 plateau)
        {
        ...
        public void DashEdges_AreWorldAnchoredInDepth_UnderTilt()
        {
            ...
            Mesh mesh = Track(BuildNorthRoad(T1StationM));
            GameObject go = Track(AttachMesh(mesh, scene.Material, "Dash_T1_Road"));
        ...
        public void DashBoundaries_ArePerpendicularAcrossTheRibbon_UnderTilt()
        {
            ...
            GameObject go = Track(AttachMesh(mesh, scene.Material, "Dash_T7_Road"));
            {
                    snap.Render(scene.UnityCamera);
            ...
            DashScene scene, SnapshotRenderer snap, Mesh mesh, string png, string what)
        {
            ...
            bag.Track(mesh);
            GameObject go = bag.Track(AttachMesh(mesh, scene.Material, what));
            snap.Render(scene.UnityCamera);
            snap.WritePng(png);
            ...
            SnapshotRenderer snap, double devicePixelRatio)
        {
            ...
            GameObject go = bag.Track(AttachMesh(mesh, scene.Material, "Dash_T2_Road"));
            {
                snap.Render(scene.UnityCamera);
