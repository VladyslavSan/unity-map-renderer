...
namespace MapRenderer.Core.View
{
    ...
    public readonly struct SymbolPlacementTelemetrySnapshot
    {
        ...
        /// <summary>B-3: symbols skipped by the pre-projection horizon/distance cull on the last Tick — never
        ...
        /// used to project then discard. Watch it against <see cref="InputSymbolCount"/> to see the gate's reach
        /// on an overzoomed view.</summary>
        public int ZoomCulledSymbols { get; init; }

        /// <summary>§1.5 companion to <see cref="SymbolStoreTelemetrySnapshot.CoverageDroppedSymbols"/>: symbols
        ...
        /// for symbols nobody can see. This is deliberately the RAW map size — never redefine it as a filtered or
        /// epsilon-thresholded count, which would hide exactly the regression it exists to expose (a fading-IN
        /// symbol legitimately holds a sub-epsilon value).</summary>
        public int LiveFadeSymbolCount { get; init; }

        /// <summary>R1: CUMULATIVE heavy rebuilds of the native symbol mirror since startup — bumped once per real
