...
{
    /// <summary>
    /// The PRE-projection label far-distance cull. In a tilted view the far half of the frustum compresses a huge
    /// ground area into a thin band at the horizon, so most labels there pile up, get collision-discarded, and
    /// jitter (their projection is numerically unstable as depth → the far plane). Testing a label's distance from
    /// the CAMERA against a cull distance BEFORE projecting it lets the placement pass skip its whole staging
    /// (project + collision-box build) for labels that would be discarded or unstable anyway.
    ///
    ...
    /// fraction pulls labels in closer than the full frustum depth. Distance is 3-D render-space length,
    ...
    /// </summary>
    public static class LabelFarPlaneCull
    {
        /// <summary>
        ...
        /// disables the cull (returns false for every label) — the safe fallback for a mis-wired caller (degrades
        ...
        /// AFTER the double subtract, mirroring <see cref="HorizonCull"/> / <c>LabelScreenProjection</c> so the
        /// cull decision agrees with where the label projects. A rotation, so it preserves the camera→anchor
            ...
            double      cullDistanceMeters)
        {
            ...
            // HorizonCull / LabelScreenProjection use, so the cull agrees with where the label projects. cameraRelative
