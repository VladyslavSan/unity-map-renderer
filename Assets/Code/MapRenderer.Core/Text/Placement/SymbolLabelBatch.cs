...
{
    /// <summary>
    /// The blittable, order-preserving Structure-of-Arrays for one collected label set — the per-frame placement
    /// source of truth (Lever C step 2). Built ONCE per collected-set change (keyed on
    /// <c>SymbolTileLabelStore.Version</c>) by converting the managed <see cref="LabelInstance"/> carriers at the
    ...
    public sealed class SymbolLabelBatch
    {
        ...
        // A SEPARATE flag from RecordDeparting: a coverage-fading tile is still ACTIVE (loaded, in cover) — only
        // its on-screen coverage crossed below threshold. Conflating it with RecordDeparting (bound to "tile
        // unloaded→cached") would assert a lifecycle lie — see LabelTileCoverageFilter's fade-vs-drop classifier.
        public bool[]    RecordCoverageFading = Array.Empty<bool>(); // per record → its tile's coverage crossed below threshold (fade OUT, don't pop)
            ...
            bool departing, bool coverageFading)
        {
            ...
            Grow(ref RepAnchor, Count); Grow(ref RecordDeparting, Count); Grow(ref RecordCoverageFading, Count);
            Kinds[Count] = kind; Detail[Count] = detail; WorldStart[Count] = worldStart; WorldCount[Count] = worldCount;
            RepAnchor[Count] = repAnchor; RecordDeparting[Count] = departing; RecordCoverageFading[Count] = coverageFading;
            return Count++;
        }
