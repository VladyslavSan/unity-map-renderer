...
// namespace-collision trap documented in SymbolBox.cs's header.
...
{
    /// <summary>
    /// A reusable uniform screen-space grid that accelerates <see cref="SymbolCollision.SelectSurvivors"/>
    /// from O(n²) to ~O(n·k) (k = local symbol density). It is a **pure prefilter**: it only narrows WHICH
    /// already-placed blockers a candidate is overlap-tested against — the accept/reject decision stays the
    /// exact same <see cref="SymbolCollision.Overlaps"/> call, so the grid-accelerated survivor set is
    ...
    /// candidate; at zoom-14-big-city symbol counts (thousands) that O(n²) pass measured ~100 ms
    ...
    /// only on warm-up (a larger viewport / more symbols than ever seen) and are reset in place otherwise.
    /// A blocker is stored as one linked-list node PER grid cell its AABB covers (a wide symbol lands in
    /// several cells); duplicate hits across cells are harmless — <see cref="SymbolCollision.Overlaps"/> is
    /// idempotent and the query breaks on the first overlap.</para>
    /// </summary>
    public sealed class SymbolCollisionGrid
    {
        // Target cell edge in logical px — roughly one text symbol tall; a symbol spans a small handful of
        ...
        /// </summary>
        public void Reset(SymbolBox[] boxes, int count)
        {
        ...
        public bool OverlapsAny(in SymbolBox box, SymbolBox[] boxes)
        {
            ...
            for (int cy = cy0; cy <= cy1; cy++)
            {
                ...
                for (int cx = cx0; cx <= cx1; cx++)
                {
                    ...
                    {
                        if (SymbolCollision.Overlaps(in box, in boxes[_nodeBox[node]])) return true;
                    }
                }
            }
            return false;
        ...
        public void Insert(int sortedBoxIndex, in SymbolBox box)
        {
            int cx0 = CellX(box.Min.x), cx1 = CellX(box.Max.x);
            int cy0 = CellY(box.Min.y), cy1 = CellY(box.Max.y);
            for (int cy = cy0; cy <= cy1; cy++)
