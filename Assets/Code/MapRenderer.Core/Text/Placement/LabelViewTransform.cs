...
// LabelScreenProjection.cs's header for the same trap.
...
{
    /// <summary>
    /// W3 — the four per-frame values <see cref="LabelScreenProjection.TryProjectPoint"/> needs, carried as
    ...
    ///
    /// <para><b>Why the MATRIX and not clip <c>w</c>.</b> <see cref="LabelStagingMath.StageCurved"/>'s doc
    ...
    /// local at the one call site (<c>LabelPlacementSystem.TickCore</c>) and already flow into the projection
    ...
    /// </summary>
    public readonly struct LabelViewTransform
    {
        ...
        /// projected <c>clip.w</c> zero, which <see cref="LabelScreenProjection.TryProjectPoint"/> rejects on
