...
namespace MapRenderer.Core.Text.Placement
{
    ...
    /// permutation-invariant, sort-key-driven survivor selection over a set of <see cref="SymbolBox"/>es.
    ///
    ...
    /// keys resolve by a stable total-order tiebreak — <see cref="SymbolBox.FeatureIndex"/> then
    /// <see cref="SymbolBox.TileKey"/> — so the survivor set is deterministic and independent of the input
    /// order (T1(a),(e)). The overlap flags refine the rule: <see cref="SymbolBox.AllowOverlap"/> places
    /// unconditionally; <see cref="SymbolBox.IgnorePlacement"/> places but never blocks a later symbol.</para>
    ///
    ...
    {
        /// <summary>
        /// The total placement order: LOWER <see cref="SymbolBox.SortKey"/> first (MapLibre priority),
        /// then LOWER <see cref="SymbolBox.FeatureIndex"/>, then LOWER <see cref="SymbolBox.TileKey"/>. The
        /// tiebreak makes the order TOTAL — no two distinct symbols compare equal (feature index + tile key
        /// are unique per symbol) — so the greedy winner is deterministic even for equal sort keys (T1(e)).
        /// </summary>
        public static int ComparePlacementOrder(in SymbolBox a, in SymbolBox b)
        {
        ...
        /// for SORTED position <c>i</c> (survivor identity is <c>boxes[i].SymbolIndex</c>) and returns the
        ...
        public static int SelectSurvivors(SymbolBox[] boxes, int count, bool[] survivor)
        {
        ...
        /// the brute-force <see cref="SelectSurvivors(SymbolBox[],int,bool[])"/> (same sort, same greedy rule,
        ...
        /// </summary>
        public static int SelectSurvivors(SymbolBox[] boxes, int count, bool[] survivor, SymbolCollisionGrid grid)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                // Place if the symbol ignores collision, OR it overlaps no already-placed blocker (AllowOverlap
                // short-circuits the query — an allow-overlap symbol is never tested, matching the reference).
                bool place = boxes[i].AllowOverlap || !grid.OverlapsAny(in boxes[i], boxes);
                survivor[i] = place;
                if (place)
                {
                    survivors++;
                    // A placed symbol blocks later ones UNLESS it ignores placement — so only non-ignore
        ...
        /// <summary>
        /// The total placement order for <see cref="SymbolCandidate"/>s — identical rule to the
        /// <see cref="SymbolBox"/> overload (lower <see cref="SymbolCandidate.SortKey"/> first, then
        /// <see cref="SymbolCandidate.FeatureIndex"/>, then <see cref="SymbolCandidate.TileKey"/>).
        /// </summary>
        ...
        /// <summary>
        /// #5 (B3) — multi-box, all-or-nothing survivor selection over a set of <see cref="SymbolCandidate"/>s
        /// (each spanning a contiguous range of the flat <paramref name="boxes"/> array), UNIFIED across point
        /// (1-box) and curved along-line (N-box) symbols so a road name and a city name collide in the SAME
        ...
        /// candidate iff <see cref="SymbolCandidate.AllowOverlap"/> OR every one of its boxes overlaps no
        ...
        /// <c>candidates[i].SymbolIndex</c>) and returns the survivor count. Zero managed allocation (T4).
        ///
        /// <para><b>All-or-nothing / no self-block:</b> a candidate's boxes are TESTED (all must be free) and
        /// only THEN inserted, so a curved symbol's adjacent glyph boxes — which naturally overlap — never
        /// block one another. One colliding glyph drops the WHOLE symbol; a placed symbol blocks across its
        /// whole run.</para>
        ///
        /// <para><b>Stage C — optional halves.</b> <see cref="SymbolCandidate.OptionalBoxMask"/> relaxes
        /// all-or-nothing per BOX (<c>icon-optional</c>/<c>text-optional</c>): a masked box that overlaps is
        /// recorded in <see cref="SymbolCandidate.DroppedBoxMask"/> and neither inserted nor drawn, while the
        ...
        /// make the pair block itself. Mask 0 (every lone symbol, every curved symbol, every pair whose style
        ...
        /// <param name="boxes">The flat box pool every candidate's <see cref="SymbolCandidate.BoxStart"/> range
        ...
        public static int SelectSurvivors(
            SymbolCandidate[] candidates, int candidateCount,
            SymbolBox[] boxes, int boxCount,
            bool[] survivor, SymbolCollisionGrid grid)
        {
        ...
        // at zoom-14-big-city symbol counts (thousands, not the "tens to low hundreds" the first cut assumed).
        // Not stable — irrelevant here: ComparePlacementOrder is a TOTAL order (no two distinct symbols tie),
        // so the sorted result is unique regardless of stability.
        private static void Sort(SymbolBox[] a, int n)
        {
        ...
        private static void SiftDown(SymbolBox[] a, int root, int n)
        {
            while (true)
            {
                int child = 2 * root + 1;
        ...
        // Candidate heapsort — identical structure to the SymbolBox Sort/SiftDown above, over the candidate
        ...
        private static void Sort(SymbolCandidate[] a, int n)
        {
        ...
        // input-/mirror-order dependent. SymbolStagingMath.SanitizeSortKey normalizes non-finite baked keys at
        // candidate build, so this can NEVER fire in production; it goes RED for a future un-normalized candidate
        // path. Compiled out unless MAPRENDERER_SYMBOL_ASSERTS (a dormant, dev-build-only [Conditional] guard — the
        ...
        [System.Diagnostics.Conditional("MAPRENDERER_SYMBOL_ASSERTS")]
        private static void AssertSortKeysFinite(SymbolCandidate[] a, int n)
        {
            for (int i = 0; i < n; i++)
                if (!(math.abs(a[i].SortKey) <= float.MaxValue))
                    throw new InvalidOperationException(
                        $"SymbolCollision: non-finite candidate SortKey ({a[i].SortKey}) at sorted index {i} — " +
                        "normalize it at candidate build (SymbolStagingMath.SanitizeSortKey, Blocker B1).");
        }

        private static void SiftDown(SymbolCandidate[] a, int root, int n)
        {
            while (true)
            {
                int child = 2 * root + 1;
