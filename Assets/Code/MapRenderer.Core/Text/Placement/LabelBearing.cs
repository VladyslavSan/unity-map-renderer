...
{
    /// <summary>
    /// The single owner of the SIGN conventions for label orientation: the map-bearing sign (the billboard
    ...
    /// label routes through here, so each "which way does this turn" question is ONE place, not sign math
    ...
    {
        /// <summary>The map-bearing sign for map-aligned label orientation (the <c>text-rotation-alignment:map</c>
        /// billboard and the <c>text-translate-anchor:map</c> offset). +1 = a map heading of θ (CW from north)
        /// turns map-aligned labels by +θ in the screen's (y-up) frame.
        ///
        ...
        /// measures ink CENTROID, a bounding box being direction-blind. It asserts the label turns +45°
        ...
        /// green, including <c>LabelBearingTests</c> (which multiplies by this very constant) and
        /// <c>LabelPlacementStructureTests</c>'s bearing case (which asserts the billboard rotates, not which
        ...
        /// (<c>LabelStagingMath.AppendPointHalf</c>), the along-line path rides it out on
        ...
        /// (<c>SymbolLabel</c> → <c>LabelInstance</c> → the stage inputs) and flips exactly once, at the
        ...
        ///
        /// <para>Written <c>0f - x</c> rather than <c>-x</c> so an UNROTATED label stays at exactly
        /// <c>+0f</c>: <c>-0f</c> is a different bit pattern, it reaches <c>BuildWorldQuad</c> on every curved
        /// TEXT label (which leaves <c>icon-rotate</c> at 0), and <c>sincos(-0f)</c> yields <c>sin = -0f</c>,
