...
namespace MapRenderer.Tests.Style
{
    ...
    public class StyleLayerEagerParseTests
    {
        ...
        /// <summary>
        /// <c>StyleParser.ParseLayer</c> sets <c>Hidden</c> only for <c>visibility: "none"</c>. Any other
        /// value — including an absent layout, an absent key, and a garbage string — is "visible", which the
        /// spec's default gives for free.
        /// </summary>
        /// <param name="layoutJson">The layer's <c>layout</c> block, or <c>null</c> to omit it entirely.</param>
        [TestCase("{\"visibility\":\"none\"}",    true,  TestName = "Visibility_None_Hides")]
        [TestCase("{\"visibility\":\"visible\"}", false, TestName = "Visibility_Visible_Draws")]
        [TestCase("{}",                            false, TestName = "Visibility_AbsentKey_Draws")]
        [TestCase("{\"visibility\":\"NONE\"}",    false, TestName = "Visibility_WrongCase_Draws")]
        [TestCase(null,                            false, TestName = "Visibility_NoLayoutBlock_Draws")]
        public void ParseLayer_Visibility_SetsHidden(string layoutJson, bool expectedHidden)
        {
    ...
    ""layers"": [ {{ ""id"": ""f0"", ""type"": ""fill"", ""source"": ""s"", ""source-layer"": ""f0"", {layout}
        ""paint"": {{ ""fill-color"": [""rgba"",102,153,204,1] }} }} ]
            ...
            Assert.AreEqual(expectedHidden, doc.Layers[0].Hidden,
                $"layout {layoutJson ?? "(absent)"} must parse to Hidden={expectedHidden}. Only the exact "
