...
namespace MapRenderer.Tests.Style
{
    ...
    ""layers"": [
        ...
        /// SURVIVE: text-halo-color's carrier evaluates Constant AND Zoom, so !DependsOnFeature is the
        /// right freeness rule for it, unlike text-color. RED-verify: apply the Constant-only arm to
        /// text-halo-color too.</summary>
        [Test]
        public void SymbolLayer_ZoomKindHaloColorChange_Survives()
        {
            var oldStyle = SymbolStyleDoc(
            ...
            Assert.IsTrue(WholeDocumentGate.AllLayersSurvive(oldStyle, newStyle),
                "a Zoom-kind text-halo-color change must survive — the halo trio each guard on " +
                "DependsOnFeature, and Zoom does not depend on feature data.");
        }
