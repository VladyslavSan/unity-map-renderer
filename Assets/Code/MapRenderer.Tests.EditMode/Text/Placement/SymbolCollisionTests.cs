...
{
    /// <summary>
    /// S20 Slice 2 — THE decisive test (stage doc §4 T1): <see cref="SymbolCollision.SelectSurvivors"/>
    /// produces a NAMED, permutation-invariant, sort-key-driven survivor set. Every assertion is
    /// OUTCOME-based (WHICH symbols survive, by <see cref="SymbolBox.SymbolIndex"/> — not a count), so the
    ...
    {
        // A box at [minX,maxX]x[minY,maxY] with the given placement key/flags. SymbolIndex == featureIndex
        ...
        // returns the set of surviving SymbolIndex values.
        private static HashSet<int> Survivors(IReadOnlyList<SymbolBox> input)
        {
            ...
            int n = SymbolCollision.SelectSurvivors(boxes, boxes.Length, flags);

            var set = new HashSet<int>();
            for (int i = 0; i < boxes.Length; i++) if (flags[i]) set.Add(boxes[i].SymbolIndex);
        ...
        // ── (b) sort-key drives the winner: swapping two overlapping symbols' keys flips the survivor ───
        [Test]
        ...
        // ── text-ignore-placement: a placed-but-non-blocking symbol lets a later overlapping one through ─
        [Test]
        public void SelectSurvivors_IgnorePlacement_DoesNotBlockLaterSymbols()
        {
        ...
        // count boxes with unique SymbolIndex/FeatureIndex; positions/sizes chosen so wide boxes cover several
        ...
        private static SymbolBox[] RandomBoxes(int count, int seed, FlagMix mix)
        {
        ...
        private static HashSet<int> SurvivorSet(SymbolBox[] boxes, bool[] flags, int count)
        {
            var set = new HashSet<int>();
            for (int i = 0; i < count; i++) if (flags[i]) set.Add(boxes[i].SymbolIndex);
            return set;
        }
        ...
        public void SelectSurvivorsGrid_MatchesBruteForce_OverRandomizedInputs()
        {
            ...
            {
                SymbolBox[] input = RandomBoxes(count, seed, mix);
                // SelectSurvivors sorts IN PLACE, so each path gets its own copy.
                var bruteBoxes = (SymbolBox[])input.Clone();
                var gridBoxes = (SymbolBox[])input.Clone();
                var bruteFlags = new bool[count];
                var gridFlags = new bool[count];

                int nBrute = SymbolCollision.SelectSurvivors(bruteBoxes, count, bruteFlags);
                int nGrid = SymbolCollision.SelectSurvivors(gridBoxes, count, gridFlags, grid);
        ...
        public void SelectSurvivorsGrid_ReusedInstance_StaysCorrectAcrossVaryingInputs()
        {
            ...
            {
                SymbolBox[] input = RandomBoxes(count, seed: 2024 + count, mix: FlagMix.Both);
                var bruteBoxes = (SymbolBox[])input.Clone();
                var gridBoxes = (SymbolBox[])input.Clone();
                var bruteFlags = new bool[count];
                var gridFlags = new bool[count];

                SymbolCollision.SelectSurvivors(bruteBoxes, count, bruteFlags);
                SymbolCollision.SelectSurvivors(gridBoxes, count, gridFlags, grid);
