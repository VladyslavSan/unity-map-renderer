...
// LabelScreenProjection (Tools/core-tests-verified pure math) against the REAL camera pipeline it's
...
{
    /// <summary>
    /// S20 T2 (Core-vs-real-camera pin): a synthetic world anchor's <see cref="LabelScreenProjection.TryProjectAnchor"/>
    ...
    /// </summary>
    [TestFixture]
    public class LabelScreenProjectionUnityTests
    {
        ...
        {
            var camGo = new GameObject("LabelProjection_TestCamera");
        ...
        public void TryProjectAnchor_MatchesRealCameraWorldToScreenPoint()
        {
            ...
            try
            {
                ...
                Assert.AreEqual(expectedScreen.x, screenPx.x, 0.5f,
                    $"LabelScreenProjection.x ({screenPx.x:F2}) must match Camera.WorldToScreenPoint.x ({expectedScreen.x:F2})");
                Assert.AreEqual(expectedScreen.y, screenPx.y, 0.5f,
                    $"LabelScreenProjection.y ({screenPx.y:F2}) must match Camera.WorldToScreenPoint.y ({expectedScreen.y:F2})");
            }
            finally
            {
