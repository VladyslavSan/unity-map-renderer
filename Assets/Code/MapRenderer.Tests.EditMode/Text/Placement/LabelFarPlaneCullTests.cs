...
{
    /// <summary>The Core distance math for the pre-projection far-plane label cull: a label farther from the camera
    ...
    [TestFixture]
    public class LabelFarPlaneCullTests
    {
        ...
        public void WithinDistance_Survives_BeyondDistance_Culled()
        {
            ...
            Assert.IsFalse(LabelFarPlaneCull.IsCulled(near, Origin, float3x3.identity, cam, 500.0),
                "300 m < 500 m cull distance ⇒ kept");
            Assert.IsTrue(LabelFarPlaneCull.IsCulled(far, Origin, float3x3.identity, cam, 500.0),
        ...
        public void DistanceIsMeasuredFromCamera_NotOrigin()
        {
            ...
            Assert.IsFalse(LabelFarPlaneCull.IsCulled(anchor, Origin, float3x3.identity, camNear, 500.0),
                "camera 300 m from the anchor ⇒ kept, though it is 700 m from the origin");
            Assert.IsTrue(LabelFarPlaneCull.IsCulled(anchor, Origin, float3x3.identity, camFar, 500.0),
                "camera 1100 m from the anchor ⇒ culled");
        }

        /// <summary>A non-positive cull distance disables the cull (keep every label) — the safe fallback for a
        ...
        public void NonPositiveDistance_DisablesCull()
        {
            ...
            Assert.IsFalse(LabelFarPlaneCull.IsCulled(farAway, Origin, float3x3.identity, double3.zero, 0.0),
                "0 ⇒ cull disabled");
            Assert.IsFalse(LabelFarPlaneCull.IsCulled(farAway, Origin, float3x3.identity, double3.zero, -1.0),
        ...
        public void RebaseRotation_IsAppliedToAnchor()
        {
            ...
            Assert.IsFalse(LabelFarPlaneCull.IsCulled(anchor, Origin, float3x3.identity, cam, 400.0),
            ...
            var ry180 = new float3x3(-1f, 0f, 0f,
                                      0f, 1f, 0f,
                                      0f, 0f, -1f);
            Assert.IsTrue(LabelFarPlaneCull.IsCulled(anchor, Origin, ry180, cam, 400.0),
