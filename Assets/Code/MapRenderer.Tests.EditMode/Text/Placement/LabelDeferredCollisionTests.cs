...
// functional consequence pinned here is a ONE-TICK verdict latency — see LabelPlacementSystem.HarvestCollision's
...
{
    [TestFixture]
    public class LabelDeferredCollisionTests
    {
        ...
        private static LabelInstance Point(double3 anchor, float sortKey, string text, int feature, float4 color)
            => new LabelInstance
            {
        ...
        // LabelPlacementDemoProductionFlipTests.cs uses to pin which label's own colour reached the mesh.
        private static float3 FirstVertexColor(LabelPlacementSystem system)
        {
        ...
        private static void AssertColorMatches(LabelPlacementSystem system, LabelInstance label, string because)
        {
            float3 actual = FirstVertexColor(system);
            float4 expected = LabelPlacementSystem.LinearColor(label);
        ...
        public void Collision_VerdictAppliesOneFrameLate_IncumbentHoldsUntilTheNextTick()
        {
            ...
            var a = Point(h.Origin, sortKey: 10f, text: "A", feature: 0, color: red);
