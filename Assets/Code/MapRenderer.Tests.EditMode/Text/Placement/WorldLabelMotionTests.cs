...
// T3 regression tooth (world-anchored-labels-design.md §10 T3, §11 A0): build the NEW world-anchored
...
// anchor, landing near the ANALYTIC expected screen position (LabelScreenProjection.TryProjectPoint at
...
// screen pixel baked once by LabelScreenProjection + BillboardMath in C#, so an OLD-path mesh built by a
// SINGLE LabelPlacementSystem.Tick and then re-rendered after a camera pan WITHOUT a second Tick is
...
{
    [TestFixture]
    public class WorldLabelMotionTests
    {
        ...
        public void WorldPath_TracksWorldAnchor_AfterCameraPan()
        {
            ...
            var camGo = new GameObject("WorldLabelMotion_TestCamera");
            ...
            try
            {
                ...
                presenterGo = new GameObject("WorldLabelMotion_Presenter");
                ...
                // bottom-origin (LabelScreenProjection's y-up convention), so a screen-Y increase (glyph
        ...
        //    anchor-Y against the SAME analytic LabelScreenProjection delta the longitude case pins anchor-X
        ...
        public void WorldPath_TracksWorldAnchor_AfterLatitudeCameraPan()
        {
            ...
            var camGo = new GameObject("WorldLabelMotionLat_TestCamera");
            var uCam = camGo.AddComponent<Camera>();
            uCam.targetTexture = new RenderTexture(Size, Size, 0);
            uCam.clearFlags = CameraClearFlags.SolidColor;
            uCam.backgroundColor = Color.white;
            ...
            try
            {
                ...
                presenterGo = new GameObject("WorldLabelMotionLat_Presenter");
                var meshFilter = presenterGo.AddComponent<MeshFilter>();
                var meshRenderer = presenterGo.AddComponent<MeshRenderer>();
                meshFilter.sharedMesh = worldMesh;
                meshRenderer.sharedMaterial = worldMaterial;
        ...
        private static Mesh BuildOneGlyphWorldMesh(in SymbolQuad quad, float textSizePx, float3 colorRgb)
        {
            ...
            var mesh = new Mesh { name = "WorldLabelMotion_OneGlyph" };
            try
            {
