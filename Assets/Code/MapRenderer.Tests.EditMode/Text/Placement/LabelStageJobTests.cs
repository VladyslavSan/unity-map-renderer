...
namespace MapRenderer.Tests.Text.Placement
{
    ...
    public class LabelStageJobTests
    {
            ...
            LabelViewTransform view = default)
        {
            ...
            int bc = 0, qc = 0, ec = 0;
            int staged = LabelStagingMath.StageCurved(in s, screen, depth, valid, world, worldUps, glyphs, anchors, fadeIds, wasPlaced,
                // W3: `view` reaches BOTH arms identically (Native assigns the same value to
                // LabelStageJob.View), so the differential compares the same transform on each side. The
        ...
        private static Result Native(in CurvedStageInput s, float2[] screen, float[] depth, byte[] valid,
            double3[] world, float3[] worldUps, CurvedGlyph[] glyphs, LineAnchor[] anchors, long[] fadeIds, byte[] wasPlaced, float bearing,
            LabelViewTransform view = default, float metresPerLogicalPixel = 0f)
        {
            ...
            new LabelStageJob
            {
                ...
                // ASYMMETRY the two arms must bridge explicitly: LabelStageJob.Execute PATCHES
                // s.MetresPerLogicalPixel from this per-frame field (LabelStageJob.cs:178), so the value
        ...
        /// W3 teeth read boxes produced by <c>LabelStageJob</c>, which is
        ...
        public void BurstStage_MatchesManaged_MapPitchedCurved()
        {
            ...
            var tileOriginRender = new double3(1_250_000.0, 0.0, -430_000.0);
            // The WORLD path must be in METRES, and long enough to hold the label: under map pitch the walk
            ...
            // label simply does not fit, which reads as "did not stage" rather than as a failure.
