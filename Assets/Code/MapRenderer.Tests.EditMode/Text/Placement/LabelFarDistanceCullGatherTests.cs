// Unity EditMode only — needs a real Camera/Material + the production Tick path (via TickLabels). NOT registered
// in core-tests.csproj (the Core distance math is pinned engine-free in LabelFarPlaneCullTests).
//
// The pre-projection far-distance cull, WIRED through the gather loop: a label farther from the camera than
// LabelMaxDistanceFraction × Camera.farClipPlane is hard-skipped in GatherSymbolPoints (never projected/staged/
// collided) and attributed to the distance bucket (LastDistanceCulledCount). These teeth pin the CONFIGURABLE
// knob — the SAME label crosses the cull boundary when the fraction changes, with the far plane held fixed.
//
...
{
    [TestFixture]
    public class LabelFarDistanceCullGatherTests
    {
        ...
        private static LabelInstance Point(double3 anchor, string text, int feature)
            => new LabelInstance
            {
                AnchorRender = anchor, Placement = SymbolPlacement.Point, Layout = OneQuad(),
                Paint = new LabelPaint { TextColor = new float4(1f, 1f, 1f, 1f), Opacity = 1f },
        ...
        private sealed class Harness : System.IDisposable
        {
            ...
            public Harness()
            {
                ...
                // CameraRelativePosition left at (0,0,0): the camera sits at the look-at, so a label's distance from
        ...
        /// <summary>THE headline tooth: with the far plane fixed at 10 000 m, a label 6 000 m from the camera is
        ...
        /// distance 8 000 m &gt; 6 000 m). Same label, same far plane, only the knob moves: pins that
        /// <c>LabelMaxDistanceFraction</c> scales the cull distance as a fraction of the far plane.</summary>
        [Test]
        public void FarLabel_CulledOrStaged_TracksTheDistanceFraction()
        {
            using var h = new Harness();
            var near = Point(h.Origin, "N", 0);                                    // at the look-at ⇒ distance 0
            var far  = Point(h.Origin + new double3(6000.0, 0.0, 0.0), "F", 1);    // 6 000 m from the camera
            var labels = new List<LabelInstance> { near, far };
            h.Cam.FarPlanePolicy = new FixedFarPlane(10000.0); // CurrentFarMetres ≡ 10 000 m

            // Tight fraction: cull distance = 0.5 × 10 000 = 5 000 m ⇒ the 6 000 m label is culled.
            h.System.LabelMaxDistanceFraction = 0.5;
            ...
            // Loose fraction: cull distance = 0.8 × 10 000 = 8 000 m ⇒ the SAME label now survives.
            h.System.LabelMaxDistanceFraction = 0.8;
            h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection);
        ...
        /// <summary>The far plane is the second lever: with the fraction held at 1.0, the same 6 000 m label is
        ...
        [Test]
        public void FarLabel_CulledOrStaged_TracksTheFarPlane()
        {
            using var h = new Harness();
            var near = Point(h.Origin, "N", 0);
            var far  = Point(h.Origin + new double3(6000.0, 0.0, 0.0), "F", 1);
            var labels = new List<LabelInstance> { near, far };
            h.System.LabelMaxDistanceFraction = 1.0;
