...
// TickLabels). NOT registered in core-tests.csproj.
//
// The pre-projection ZOOM gate: a label whose style layer is out of the live camera zoom's [minzoom, maxzoom)
...
// skipped. See LabelPlacementSystem.GatherSymbolPoints / IsOutOfLiveZoom / ApplySuppression.
...
{
    [TestFixture]
    public class LabelZoomGateGatherTests
    {
        ...
        /// <summary>THE headline tooth (RED against the pre-gate code): a FRESH label whose layer is out of the
        ...
        [Test]
        public void FreshOutOfZoomLabel_HardSkippedInGather_AbsentFromCollision()
        {
            using var h = new Harness();
            var outOfZoom = BuildRenderLayer(minZoom: 10.0); // 10 > live 5.0 ⇒ out of zoom
            try
            {
                var labels = new List<LabelInstance> { Point(h.Origin, "A", 0) };
                var layers = new List<SymbolRenderLayer> { outOfZoom };

                h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection, symbolLayers: layers); // fresh — no live fade
        ...
        /// <summary>Control: the identical label under an IN-zoom layer is NOT zoom-culled — it stages as a
        ...
        [Test]
        public void InZoomLabel_NotZoomCulled_StagesAsCandidate()
        {
            using var h = new Harness();
            ...
            try
            {
                ...
                h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection, symbolLayers: layers);
        ...
        /// <summary>With NO layer list (the demo / single-material path) the zoom gate is inert — every label
        ...
        public void NoLayerList_ZoomGateInert_EverythingStages()
        {
            ...
            h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection); // symbolLayers: null

            Assert.AreEqual(1, h.System.LastCandidateCount, "no layer list ⇒ no zoom gate ⇒ the label stages");
            Assert.AreEqual(0, h.System.LastZoomCulledCount, "…and nothing is zoom-culled");
        }

        /// <summary>The fade-out-in-place exemption (guards the fall-through): a label shown while IN zoom, whose
        ...
        [Test]
        public void PreviouslyShownLabel_GoneOutOfZoom_StaysStagedFading_NotHardSkipped()
        {
            using var h = new Harness();
            ...
            try
            {
                ...
                h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection, symbolLayers: inLayers);
                h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection, symbolLayers: inLayers);
                ...
                h.System.TickLabels(in h.Frame, labels, h.Atlas, h.Projection, symbolLayers: outLayers);
