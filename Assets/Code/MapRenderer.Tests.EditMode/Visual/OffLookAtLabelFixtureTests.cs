...
// LabelPlacementSystem.Tick), off-screen GPU render + CPU readback.
// NOT registered in Tools/core-tests/core-tests.csproj.
//
// Stage P-M — the off-look-at fixture's OWN acceptance teeth (M1–M13). Read OffLookAtLabelScene's header
...
//   • any tooth whose expected value is derived from the near label's MEASURED spacing scaled by anything —
//     that is the P3a self-referential-oracle failure re-imported.
//   • (The third P-M anti-tooth, "any spacing tooth on the RECEDING label", is RETIRED: it existed because
//     the pre-W1 screen walk anchored a receding label at the screen arc midpoint, which is not the
...
{
    [TestFixture]
    public class OffLookAtLabelFixtureTests
    {
        private static OffLookAtLabelScene CreateFixture()
        ...
        /// five-glyph label carries thousands of ink pixels, so this floor only asks "did anything render
        /// here at all".</summary>
        private const int InkFloor = 200;

        // ═══════════════════════════════════════════════════════════════════════════════════════════════
        // M1–M5 — the frame geometry and the ORACLE itself. No mesh, no label, no render is read here.
        // ═══════════════════════════════════════════════════════════════════════════════════════════════
        ...
        ///
        /// <para>Does NOT prove anything about labels, staging, spacing or rendering.</para>
        ///
        ...
        public void TwoAnchors_ProjectOnScreen_WithMargin()
        {
            ...
            double2 nearPx = f.Measure(OffLookAtLabelId.CrossNear).AnchorScreenPx;
            double2 farPx  = f.Measure(OffLookAtLabelId.CrossFar).AnchorScreenPx;
        ...
        ///
        /// <para>Does NOT prove anything about labels or layout — this is the projection alone. It is the
        ...
        // ═══════════════════════════════════════════════════════════════════════════════════════════════
        // M6–M9 — the staged label geometry, read back from the built meshes.
        // ═══════════════════════════════════════════════════════════════════════════════════════════════

        /// <summary>
        /// <b>M6 — THE TOOTH THAT VALIDATES THE WHOLE READBACK PATH.</b> Proves: the world anchor the
        /// production staging baked for the near label's CENTRE glyph, read back through
        ...
        public void GlyphWorldAnchors_LandWhereTheFixtureIntended_AtTheLookAt()
            => AssertCentreGlyphLandsAtTheIntendedAnchor(OffLookAtLabelId.CrossNear, "M6 near");
        ...
        /// headline RATIO intact, because the ratios are differences within one label — so this is the one
        ...
        public void GlyphWorldAnchors_LandWhereTheFixtureIntended_AwayFromTheLookAt()
            => AssertCentreGlyphLandsAtTheIntendedAnchor(OffLookAtLabelId.CrossFar, "M6b far");

        private static void AssertCentreGlyphLandsAtTheIntendedAnchor(OffLookAtLabelId id, string what)
        {
            using var f = CreateFixture();
        ...
        /// <summary>
        /// <b>M7.</b> Proves: both cross-azimuth labels really do lie at CONSTANT view depth — every glyph of
        /// a label is within 0.5 % of that label's mean depth, at both depths. This is the property the
        /// headline measurement rests on: it is what makes the far/near comparison a pure 1/w law with no
        /// within-label foreshortening to correct for.
        ///
        /// <para>Does NOT prove that ĉ is the cross-azimuth direction in any absolute sense, and nothing about
        /// spacing.</para>
        ///
        /// <para>RED-verify: point the first reading at <c>RecedingNear</c>, a label that genuinely spans a
        /// depth range ⇒ the spread reads ~36 %, 70× the bound. NOT by rebuilding the cross labels along ĝ:
        ...
        /// </summary>
        [Test]
        public void CrossAzimuthLabels_LieAtConstantViewDepth()
        {
            using var f = CreateFixture();
            double nearSpread = RelativeDepthSpread(f.Measure(OffLookAtLabelId.CrossNear));
            double farSpread  = RelativeDepthSpread(f.Measure(OffLookAtLabelId.CrossFar));
        ...
        /// <summary>
        /// <b>M8.</b> Proves: both cross-azimuth labels staged EVERY glyph — <c>4 · GlyphCount</c> stream-0
        /// vertices on each slot mesh. A curved label whose road is too short returns 0 from
        ...
        /// readback, on every label.)</para>
        ///
        /// <para>Does NOT prove the glyphs are in the right PLACE — that is M6/M9's job.</para>
        ///
        /// <para>RED-verify: set <c>SpillMargin = 0.5</c> ⇒ the roads are shorter than the labels and both
        /// stage 0.</para>
        /// </summary>
        [Test]
        public void CurvedLabels_StageEveryGlyph_AtBothDepths()
        {
            using var f = CreateFixture();
            int expected = 4 * f.Config.GlyphCount;
            int nearCount = f.VertexCount(OffLookAtLabelId.CrossNear);
            int farCount  = f.VertexCount(OffLookAtLabelId.CrossFar);
        ...
        /// glyph gaps — within each cross-azimuth label the gaps are uniform to better than 1 %, as uniform
        ...
        /// what this stage does not assert for the far label.</para>
        ///
        /// <para>RED-verify: perturb one glyph's <c>ArcCenter</c> in the fixture ⇒ non-uniform.</para>
        /// </summary>
        [Test]
        public void GlyphSpacing_IsUniformWithinEachLabel()
        {
            using var f = CreateFixture();
            double nearNonUniformity = ScreenSpacingNonUniformity(f.Measure(OffLookAtLabelId.CrossNear));
            double farNonUniformity  = ScreenSpacingNonUniformity(f.Measure(OffLookAtLabelId.CrossFar));
        ...
        /// <summary>
        /// <b>M10 — RENDER CORROBORATION.</b> Proves: both cross-azimuth labels actually RENDER, each inside
        ...
        /// is X px TOP-DOWN — so the far label's screen area falls as <c>1/w²</c> BY DESIGN. Keeping one flat
        /// floor for both bands would assert that the far label is NOT foreshortened, i.e. it would pin the
        ...
        /// (tilt 55, ratio 2). The pre-W2 pair was taken by forcing <c>WorldLabelRenderer</c>'s
        /// <c>mapPitchCorners</c> to <c>false</c> and restoring it:
        /// <code>
        ///            near band    far band    far/near
        ///   pre-W2      1200 px     1262 px     1.052    &lt;- screen-constant size: the far label renders the
        ...
        /// is 98/50 = 1.96×.</para>
        ///
        /// <para>That conservatism is also why this tooth stays a PRESENCE check ("did the far label reach the
        ...
        {
            using var f = CreateFixture();
            f.RowBandFor(OffLookAtLabelId.CrossNear, out int nearFrom, out int nearTo);
            f.RowBandFor(OffLookAtLabelId.CrossFar,  out int farFrom,  out int farTo);
            ...
            // above the near band's level would mean the band caught a neighbouring label instead.
            ...
            // The far label must be FORESHORTENED, not merely present. The bound is the depth ratio, not 1:
        ...
        private static int FarInkFloor(OffLookAtLabelScene f)
        {
        ...
        /// <summary>
        /// <b>M11 — the DEPTH-SPANNING arm's precondition.</b> Proves: the fixture really does build a label
        ...
        /// walk from a screen walk scaled by one per-label constant.
        ///
        ...
        /// <c>RecedingFar</c>'s sits much closer to it (its label is at ~2× the depth, so the same world span
        ...
        ///
        /// <para>RED-verify: rebuild the receding labels along ĉ ⇒ the depth span collapses to 1.00 and the
        /// monotonicity is noise.</para>
        /// </summary>
        [Test]
        public void RecedingLabel_SpansAMonotonicDepthRange()
        {
            using var f = CreateFixture();
            GlyphMeasurement[] glyphs = f.Measure(OffLookAtLabelId.RecedingNear).Glyphs;
        ...
        /// point label's single staged quad carries a world anchor that projects within 2 px of where the
        ...
        ///
        /// <para>Does NOT prove anything about spacing (a point label has one quad) or about ink — the point
        /// labels are deliberately excluded from the rendered frame.</para>
        ///
        /// <para>RED-verify: offset one intended point anchor by <c>20·mpp</c> ⇒ fails by ~20 px (near) or
        /// ~10 px (far).</para>
        /// </summary>
        [Test]
        public void PointLabels_StageAtBothDepths_WhereTheOracleProjectsThem()
        {
            using var f = CreateFixture();
            int nearVerts = f.VertexCount(OffLookAtLabelId.PointNear);
            int farVerts  = f.VertexCount(OffLookAtLabelId.PointFar);
            ...
            LabelMeasurement near = f.Measure(OffLookAtLabelId.PointNear);
            LabelMeasurement far  = f.Measure(OffLookAtLabelId.PointFar);
        ...
        /// (look-at) label's mean screen spacing agrees with the model oracle within 3 %.
        ///
        ...
        ///
        /// <para><b>It asserts NOTHING about the far label — deliberately, and still.</b> The far assertion
        ...
        /// table for all six labels and the four headline ratios, so the evidence is reproducible from the
        ...
        ///
        /// <para>The headline ratios are ratios of two readings from ONE frame and ONE label pair, so any
        ...
        public void CurvedGlyphSpacing_MatchesTheModelAtTheLookAt_AndReportsBothDepths()
        {
            ...
            double nearMeasured = OffLookAtLabelScene.Mean(f.Measure(OffLookAtLabelId.CrossNear).ScreenSpacingPx);
            double nearOracle   = f.ModelPredictedScreenSpacingPx(OffLookAtLabelId.CrossNear);
        ...
        private static double RelativeDepthSpread(LabelMeasurement m)
        {
        ...
        private static double ScreenSpacingNonUniformity(LabelMeasurement m)
        {
