...
namespace MapRenderer.Tests
{
    ...
    internal sealed class TiltedGroundScene : IDisposable
    {
        ...
        /// <summary>Renders <paramref name="snapshot"/>'s off-screen target off this scene's camera, then
        /// checks the GPU-context guard.
        ///
        ...
        public void Render(SnapshotRenderer snapshot)
        {
            ...
            AssertGpuContext(snapshot);
        }

        /// <summary>The blank-control <c>Assert.Inconclusive</c> guard, verbatim from
        /// <c>LineProbeSymmetrySnapshotTests.AssertGpuContext</c> (S111): an all-black scene render plus an
        /// all-black blank render (SAME background, so the control is meaningful) means no GPU context in
        /// batch EditMode, not a genuine failure.</summary>
        public void AssertGpuContext(SnapshotRenderer snapshot)
        {
            if (!snapshot.IsAllBlack()) return;
            var blankGo = new GameObject("TiltedGroundScene_BlankCamera");
            try
            {
                var blankCam = blankGo.AddComponent<Camera>();
                blankCam.clearFlags      = CameraClearFlags.SolidColor;
                blankCam.backgroundColor = Config.BackgroundColor;
                using var blank = new SnapshotRenderer(Config.SizePx, Config.SizePx);
