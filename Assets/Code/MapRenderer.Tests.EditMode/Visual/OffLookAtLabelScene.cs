...
// LabelPlacementSystem.Tick. NOT registered in Tools/core-tests/core-tests.csproj.
//
// Stage P-M — THE OFF-LOOK-AT, MULTI-DEPTH MEASUREMENT FIXTURE.
//
// WHAT THIS IS. One rendered scene carrying SIX real labels at TWO view depths: a curved label anchored at
...
// point label at each of the two anchors. It measures, per label, the per-glyph WORLD anchors the CPU
// staging produced (read back from the built mesh), their screen positions through the LIVE camera, and
// their view depths.
//
// WHY IT EXISTS. Every previous fixture in the pitch-alignment epic anchored its label at the LOOK-AT, where
...
// tree and deliberately asserted nothing about the far label; the fix is stage W1, and it landed HERE. The
// four curved labels now carry `PitchAlignment = AlignmentMode.Map`, so `LabelStagingMath.StageCurved` lays
...
//     LineCenter (0, 0.5) anchor resolves to the road's WORLD midpoint, so a receding label DOES land at its
...
// screenSpacing_far / screenSpacing_near — a RATIO of two measurements taken in one frame from one label
...
//
// THE ORACLE IS NOT THE CODE UNDER TEST. ModelPredictedScreenSpacingPx below calls NONE of LabelStagingMath,
// PolylineArcMath, CurvedTextLayout, TextQuadLayout.Layout, BillboardMath, WorldLabelRenderer or any shader.
...
namespace MapRenderer.Tests.Visual
{
    ...
    internal enum OffLookAtDirection
    {
        ...
        /// of the label sits at ONE view depth. The headline arm (see this file's header).</summary>
        CrossAzimuth,
        ...
        /// walk scaled by one per-label constant.</summary>
        Receding,
    }

    /// <summary>The six labels this fixture stages, each on its OWN tile key (hence its own slot mesh, so no
    /// two labels' glyph vertices ever share a buffer).</summary>
    internal enum OffLookAtLabelId
    {
        CrossNear, CrossFar, RecedingNear, RecedingFar, PointNear, PointFar,
    }

    /// <summary>
    /// Configuration for <see cref="OffLookAtLabelScene"/>. A plain data carrier with
    /// object-initializer construction.
    /// </summary>
    internal sealed class OffLookAtLabelSceneConfig
    {
        ...
        /// <summary>How much longer than the label each road is, per side — guards
        /// <c>LabelStagingMath.StageCurved</c>'s <c>centerArc ± halfSpan</c> spill return. Since W1 that gate
        /// is in world METRES for a map-pitched label, so this multiplies the label's world span; the
        ...
        /// <see cref="OffLookAtLabelScene.MetresPerLogicalPixel"/>, and therefore
        /// <see cref="OffLookAtLabelScene.AdvanceWorldMetres"/>, the SAME number of metres at DPR 1 and
    ...
    /// <summary>One label's per-glyph measurements plus the consecutive-glyph spacings derived from them.</summary>
    internal readonly struct LabelMeasurement
    {
        ...
        /// <summary>The FIXTURE'S OWN intended anchor for this label (not a measured value) — the point the
        ...
        /// are built symmetric in metres, so a receding label now lands here too. F4 is retired for
        /// map-pitched labels.</para></summary>
        ...
        public LabelMeasurement(GlyphMeasurement[] glyphs, double[] worldSpacingM, double[] screenSpacingPx,
    ...
    /// </summary>
    internal sealed class OffLookAtLabelScene : IDisposable
    {
        public readonly OffLookAtLabelSceneConfig Config;
        ...
        /// <see cref="OffLookAtLabelSceneConfig.TargetDepthRatio"/>).</summary>
        ...
        /// <summary>The whole label's screen span, <c>(GlyphCount − 1) · GlyphGapScreenPx</c>.</summary>
        public readonly double LabelSpanScreenPx;

        /// <summary>The 'F' cell's own screen width at <see cref="OffLookAtLabelSceneConfig.TextSizePx"/> —
        ...
        /// <see cref="OffLookAtLabelSceneConfig.AdvanceBakedPx"/> it gives W2's equation-(6) constant,
        ...
        /// <summary>Per-label road half-lengths in metres, <c>[negative side, positive side]</c> — reported
        /// because the receding roads are capped by reachability (see <see cref="SolveGroundOffsetForScreenPx"/>).</summary>
        public readonly Dictionary<OffLookAtLabelId, double2> RoadHalfLengthsM = new();

        /// <summary>Per-label achieved road half-extents in SCREEN px, <c>[negative side, positive side]</c>.</summary>
        public readonly Dictionary<OffLookAtLabelId, double2> RoadHalfExtentsPx = new();
        ...
        /// <summary>Half-height, in rows, of the band the ink arm reads around each cross-azimuth label's
        ...
        /// <summary>W2 — the COLUMN analogue of <see cref="InkBandHalfRows"/>, for a RECEDING label. A
        /// receding label runs screen-VERTICALLY, so its ink is segmented along ROWS inside a column band.
        /// <para>The band must span the cell's CROSS-ROAD arm, which for a receding label lies along ĉ — the
        /// cross-azimuth direction, perpendicular to the view axis and therefore NOT foreshortened. That arm
        /// is the ~34-px cell height at the near label and shrinks with depth at the far one, so the same 45
        /// half-width that serves the cross arm is comfortably generous here too.</para></summary>
        public const int InkBandHalfCols = 45;

        private readonly Dictionary<OffLookAtLabelId, LabelMeasurement> _measurements = new();
        private readonly Dictionary<OffLookAtLabelId, int> _vertexCounts = new();
        // W2: the raw stream-0 vertices each label's slot mesh carried in the GEOMETRY pass, captured there
        // so a later re-tick cannot change what a tooth reads. See Vertices(id) / RenderIsolated(id).
        private readonly Dictionary<OffLookAtLabelId, WorldBillboardVertex[]> _vertices = new();
        private readonly Dictionary<OffLookAtLabelId, LabelInstance> _labelsById = new();
        private readonly LabelPlacementSystem _system;
        ...
        private OffLookAtLabelScene(
            OffLookAtLabelSceneConfig config, TiltedGroundScene scene,
            ...
            double glyphGapScreenPx, double labelSpanScreenPx, double glyphCellScreenWidthPx,
            byte[] inkPixels, LabelPlacementSystem system, TestSymbolPlan plan,
            GlyphAtlasTexture atlasF, GlyphAtlasTexture atlasA, SnapshotRenderer snapshot)
        {
            ...
            LabelSpanScreenPx         = labelSpanScreenPx;
        ...
        /// <summary>This label's per-glyph measurements, taken from the GEOMETRY pass (see
        /// <see cref="Create"/>'s two-pass note).</summary>
        public LabelMeasurement Measure(OffLookAtLabelId id) => _measurements[id];

        /// <summary>Stream-0 vertex count on this label's slot mesh — 4 per staged glyph.</summary>
        public int VertexCount(OffLookAtLabelId id) => _vertexCounts[id];

        /// <summary>W2 — this label's raw stream-0 <see cref="WorldBillboardVertex"/>s, in TL/TR/BR/BL order
        /// per glyph, exactly as <c>WorldLabelRenderer.Emit</c> wrote them, captured in the GEOMETRY pass.
        ///
        ...
        public WorldBillboardVertex[] Vertices(OffLookAtLabelId id) => _vertices[id];

        /// <summary>The inclusive ink COLUMN band around <paramref name="id"/>'s projected anchor — the
        /// column analogue of <see cref="RowBandFor"/>, for a RECEDING label (which runs screen-vertically,
        /// so its ink runs are segmented along ROWS within a band of COLUMNS).</summary>
        public void ColumnBandFor(OffLookAtLabelId id, out int colFrom, out int colTo)
        {
        ...
        /// two CROSS-AZIMUTH labels only — <see cref="Create"/>'s two-pass note explains that a receding
        /// label crosses the frame vertically by construction, so with everything drawn at once no band can
        /// say WHICH label put ink in it. The two receding labels additionally run along the same ground
        /// direction ĝ from the same origin, so at heading 0 they project into nearly the SAME columns and a
        /// column band cannot separate them either. Rendering one label at a time removes the attribution
        ...
        /// </summary>
        public byte[] RenderIsolated(OffLookAtLabelId id)
        ...
        /// SINGLE label in the frame</b> — <c>StageCurvedAnchor</c> appends box <c>g</c> and quad <c>g</c> in
        /// the same loop iteration, <c>WorldLabelRenderer.Emit</c> walks the quads in order, and
        /// <c>LabelCollision.SelectSurvivors</c> does not reorder the box pool (it sorts the CANDIDATES; the
        /// grid stores absolute box indices). Callers must still ASSERT it
        /// (<c>boxes.Length == vertices.Length / 4</c>), never assume it.</para>
        /// </summary>
        public byte[] RenderIsolated(OffLookAtLabelId id, out LabelBox[] boxes,
            out WorldBillboardVertex[] vertices, out Transform slot)
        {
            var one = new List<LabelInstance> { _labelsById[id] };
            // R3: duplicate Tick — the collision verdict is harvested one Tick late.
            _system.Tick(in _frame, _plan.Build(one), _atlasF);
            _system.Tick(in _frame, _plan.Build(one), _atlasF);
            int expected = _labelsById[id].CurvedGlyphs != null ? Config.GlyphCount : 1;
            ...
            NativeArray<LabelBox> staged = _system.LastStagedBoxes();
            boxes = new LabelBox[_system.LastBoxCount];
            for (int b = 0; b < boxes.Length; b++) boxes[b] = staged[b];

            long tileKey = _labelsById[id].TileKey;
            Assert.That(_system.TryGetWorldSlotMesh(tileKey, 0, LabelKind.Text, out Mesh mesh), Is.True,
            ...
            slot = _system.WorldSlotTransform(tileKey, 0, LabelKind.Text);
        ...
        ///
        /// <para>Valid as stated for a CROSS-AZIMUTH label (one <c>w</c> for the whole label). For a receding
        /// label it is a single-depth linearisation and is reported, never asserted.</para>
        /// </summary>
        public double ModelPredictedScreenSpacingPx(OffLookAtLabelId id)
        ...
        /// metres (see <see cref="OffLookAtLabelSceneConfig.DevicePixelRatio"/>).</para></summary>
        ...
        public void RowBandFor(OffLookAtLabelId id, out int rowFrom, out int rowTo)
        {
        ...
        /// <summary>
        /// Builds the scene, solves the two anchors, stages all six labels through the REAL
        /// <c>LabelPlacementSystem.Tick</c>, measures every slot mesh, and renders the ink frame.
        ///
        /// <para><b>TWO TICK PASSES, ONE SCENE AND ONE CAMERA.</b> Pass 1 ticks all six labels and takes every
        /// geometric measurement off the built meshes. Pass 2 re-ticks with ONLY the two cross-azimuth labels
        /// and renders — because the receding and point labels' ink sweeps across both cross-azimuth row
        /// bands (a receding label crosses the frame vertically by construction), which would make the ink
        /// arm unable to say WHICH label put ink in a band. The two passes share the camera, so their
        ...
        /// </summary>
        public static OffLookAtLabelScene Create(OffLookAtLabelSceneConfig config)
        {
        ...
        private static OffLookAtLabelScene Build(
            OffLookAtLabelSceneConfig config, TiltedGroundSceneConfig sceneConfig,
            ref TiltedGroundScene scene, ref GlyphAtlasTexture atlasF, ref GlyphAtlasTexture atlasA,
            ref LabelPlacementSystem system, ref TestSymbolPlan plan, ref SnapshotRenderer snapshot)
        {
            ...
            // ANCHOR (M12) and the point labels are excluded from the ink pass, so the mismatched atlas
            // cannot reach any reading this fixture takes.
            SymbolQuad cell;
            (atlasF, cell) = WorldCurvedAbRenderSnapshotTests.BuildGlyphF();
            TextLayoutResult layoutA;
            (atlasA, layoutA) = WorldPointEmitRenderTests.BuildGlyphA();

            double scale             = config.TextSizePx / TextQuadLayout.OneEm;
            double glyphGapScreenPx  = config.AdvanceBakedPx * scale;
            double labelSpanScreenPx = (config.GlyphCount - 1) * glyphGapScreenPx;
            ...
            var roadHalfLengths = new Dictionary<OffLookAtLabelId, double2>();
            var roadHalfExtents = new Dictionary<OffLookAtLabelId, double2>();
            var anchors = new Dictionary<OffLookAtLabelId, double3>
            {
                [OffLookAtLabelId.CrossNear]    = nearAnchor,
                [OffLookAtLabelId.CrossFar]     = farAnchor,
                [OffLookAtLabelId.RecedingNear] = nearAnchor,
                [OffLookAtLabelId.RecedingFar]  = farAnchor,
                [OffLookAtLabelId.PointNear]    = nearAnchor,
                [OffLookAtLabelId.PointFar]     = farAnchor,
            };

            var labels = new List<LabelInstance>();
            // W2: the same instances, keyed by id, so RenderIsolated can re-Tick exactly ONE of them without
            // depending on this list's construction order.
            var labelsById = new Dictionary<OffLookAtLabelId, LabelInstance>();
            var curvedIds = new[]
            {
                OffLookAtLabelId.CrossNear, OffLookAtLabelId.CrossFar,
                OffLookAtLabelId.RecedingNear, OffLookAtLabelId.RecedingFar,
            };
            // W1: the label's span as the staging gate now measures it — WORLD metres. `labelSpanScreenPx` is
            ...
            double labelSpanArcM      = labelSpanScreenPx * metresPerLogicalPx;
            ...
            // exact ×1.0 at DPR 1.
            double targetHalfRoadPx = config.SpillMargin * labelSpanScreenPx * config.DevicePixelRatio;
            ...
            double recedingHalfLenM = config.SpillMargin * labelSpanArcM;
            ...
            {
                bool cross = id == OffLookAtLabelId.CrossNear || id == OffLookAtLabelId.CrossFar;
                double3 dir = cross ? cHat : gHat;
                double3 anchor = anchors[id];
                double anchorDepth = cross
                    ? (id == OffLookAtLabelId.CrossNear ? wNear : wFar)
                    : (id == OffLookAtLabelId.RecedingNear ? wNear : wFar);
                ...
                else
                {
                    ...
                    // eats the label) and its up-screen end must not run past the camera into non-positive
                    // view depth.
                ...
                // W1: all four curved labels are map-pitched, so StageCurved centres them at the WORLD arc
                // midpoint and its `labelSpanArc > total` / `centerArc ± halfSpan` gates are in METRES. This
                ...
                // HERE, with its numbers, rather than as a silently missing label.
                Assert.That(tMinus + tPlus, Is.GreaterThan(1.05 * labelSpanArcM),
                    $"P-M precondition ({id}): the road's total world arc " +
                    $"({tMinus:F0} + {tPlus:F0} m) must exceed the label's world span " +
                    $"({labelSpanArcM:F0} m) or StageCurved drops the label at its only anchor. Achieved " +
                ...
                var curvedLabel = new LabelInstance
                {
                    ...
                    // it these labels keep taking the pre-W1 screen walk and the stage measures nothing.
                    ...
                    // its LabelInstances, so it states the resolved value directly.)
                    PitchAlignment = AlignmentMode.Map,
                    PathRender   = new[] { origin + pathA, origin + pathB },
                    ...
                    // and a fixture silently loses labels.
                    AllowOverlap = true,
                    FeatureIndex = (int)id,
                    TileKey      = TileKeyFor(baseTile, id),
                };
                labels.Add(curvedLabel);
                labelsById[id] = curvedLabel;
            }

            foreach (OffLookAtLabelId id in new[] { OffLookAtLabelId.PointNear, OffLookAtLabelId.PointFar })
            {
                var pointLabel = new LabelInstance
                {
                    AnchorRender = origin + anchors[id],
                    UpRender     = worldUp,
                    Layout       = layoutA,
                    Paint        = LabelPaint.Default,
                    Text         = id.ToString(),
            ...
            var vertexCounts = new Dictionary<OffLookAtLabelId, int>();
            var slotVerticesById = new Dictionary<OffLookAtLabelId, WorldBillboardVertex[]>();
            byte[] inkPixels;
            {
                // PASS 1 — geometry. R3: duplicate Tick, the collision verdict is harvested one Tick late.
                system.Tick(in frame, plan.Build(labels), atlasF);
                system.Tick(in frame, plan.Build(labels), atlasF);
                Assert.That(plan.CollectedCount, Is.EqualTo(labels.Count),
                    $"P-M precondition: the collect must yield all {labels.Count} labels (got " +
                ...
                var perLabel = new StringBuilder();
                foreach (OffLookAtLabelId id in (OffLookAtLabelId[])Enum.GetValues(typeof(OffLookAtLabelId)))
                {
                    int quads = system.TryGetWorldSlotMesh(TileKeyFor(baseTile, id), 0, LabelKind.Text,
                        out Mesh slotMesh) && slotMesh != null ? slotMesh.vertexCount / 4 : 0;
                    perLabel.Append(" ").Append(id.ToString()).Append("=").Append(quads);
                    if (roadHalfExtents.TryGetValue(id, out double2 ext))
                        perLabel.Append(string.Format(CultureInfo.InvariantCulture,
                            " (road -{0:F1}/+{1:F1} px, label span {2:F1} px)", ext.x, ext.y, labelSpanScreenPx));
                }
                Assert.That(system.LastQuadCount, Is.EqualTo(expectedQuads),
                    ...
                    $"Per label:{perLabel}. A zero means that label spilled its road " +
                ...
                // frame (they are sized in world metres now), and W1-T2/T3/T4 read them — a receding label
                ...
                // label whose anchor defines it (see this method's two-pass note).
                var crossOnly = new List<LabelInstance> { labels[0], labels[1] };
                system.Tick(in frame, plan.Build(crossOnly), atlasF);
                system.Tick(in frame, plan.Build(crossOnly), atlasF);
                Assert.That(system.LastQuadCount, Is.EqualTo(2 * config.GlyphCount),
                    $"P-M precondition: the ink pass must stage exactly the two cross-azimuth labels " +
                ...
                foreach (OffLookAtLabelId id in new[] { OffLookAtLabelId.CrossNear, OffLookAtLabelId.CrossFar })
                {
                    LabelMeasurement again = MeasureSlot(system, cam, TileKeyFor(baseTile, id), anchors[id],
                        out _, out _);
            ...
            var built = new OffLookAtLabelScene(config, scene, gHat, cHat, nearAnchor, farAnchor, wNear, wFar,
                mpp, absP11, viewportHeightPx, glyphGapScreenPx, labelSpanScreenPx, cellWidthScreenPx,
                inkPixels, system, plan, atlasF, atlasA, snapshot);
            built._frame = frame;
            foreach (KeyValuePair<OffLookAtLabelId, LabelInstance> kv in labelsById)
                built._labelsById[kv.Key] = kv.Value;
            foreach (KeyValuePair<OffLookAtLabelId, WorldBillboardVertex[]> kv in slotVerticesById)
                built._vertices[kv.Key] = kv.Value;
            foreach (KeyValuePair<OffLookAtLabelId, LabelMeasurement> kv in measurements)
                built._measurements[kv.Key] = kv.Value;
            foreach (KeyValuePair<OffLookAtLabelId, int> kv in vertexCounts)
                built._vertexCounts[kv.Key] = kv.Value;
            foreach (KeyValuePair<OffLookAtLabelId, double2> kv in roadHalfLengths)
                built.RoadHalfLengthsM[kv.Key] = kv.Value;
            foreach (KeyValuePair<OffLookAtLabelId, double2> kv in roadHalfExtents)
                built.RoadHalfExtentsPx[kv.Key] = kv.Value;
            return built;
        }

        /// <summary>Each label gets its OWN tile key (the containing z14 tile's X + the label ordinal), so the
        /// six labels land in six separate slot meshes and no two labels' glyph vertices ever interleave in
        /// one buffer. The far labels additionally carry a genuinely large Level-1 RTC bake — their anchors
        ...
        /// </summary>
        private static LabelMeasurement MeasureSlot(LabelPlacementSystem system, UnityEngine.Camera cam,
            ...
            out WorldBillboardVertex[] slotVertices)
        {
            ...
            Transform slot = system.WorldSlotTransform(tileKey, 0, LabelKind.Text);
            ...
            return new LabelMeasurement(glyphs, worldSpacing, screenSpacing, intendedAnchorUnity,
            ...
            double maxGroundFromLookAtM, out double reachedPx)
        {
            ...
            // the vanishing line makes arbitrarily expensive, and the whole label was culled before staging.
        ...
        /// <summary>The stage's evidence table, for <c>TestContext.WriteLine</c>: per gap, for every label,
        ...
        public string FormatMeasurementTable()
        {
            ...
            sb.AppendLine(string.Format(c,
                ...
                Config.TextSizePx / TextQuadLayout.OneEm, GlyphGapScreenPx, LabelSpanScreenPx,
                GlyphCellScreenWidthPx, Config.SpillMargin));
            sb.AppendLine(string.Format(c,
                "anchors (device px): near=({0:F1}, {1:F1})  far=({2:F1}, {3:F1})",
                Measure(OffLookAtLabelId.CrossNear).AnchorScreenPx.x,
                Measure(OffLookAtLabelId.CrossNear).AnchorScreenPx.y,
                Measure(OffLookAtLabelId.CrossFar).AnchorScreenPx.x,
                Measure(OffLookAtLabelId.CrossFar).AnchorScreenPx.y));
            sb.AppendLine();

            foreach (OffLookAtLabelId id in (OffLookAtLabelId[])Enum.GetValues(typeof(OffLookAtLabelId)))
            {
                LabelMeasurement m = Measure(id);
                bool cross = id == OffLookAtLabelId.CrossNear || id == OffLookAtLabelId.CrossFar;
                bool point = id == OffLookAtLabelId.PointNear || id == OffLookAtLabelId.PointFar;
                ...
                // closed form. On a cross-azimuth label that equals the anchor-depth oracle to within M7's
                // 0.5 % bound; on a receding label it is the only form that means anything, since the label
                ...
                // is the column that tells a per-GLYPH world walk from a screen walk scaled by one per-label
                // constant — on an iso-depth label the two are identical, so the receding rows are the ones
                // that mean something.
                sb.AppendLine(
            ...
            LabelMeasurement near = Measure(OffLookAtLabelId.CrossNear);
            LabelMeasurement far  = Measure(OffLookAtLabelId.CrossFar);
            ...
            double nearOracle = ModelPredictedScreenSpacingPx(OffLookAtLabelId.CrossNear);
            double farOracle  = ModelPredictedScreenSpacingPx(OffLookAtLabelId.CrossFar);
