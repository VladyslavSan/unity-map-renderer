// Unity EditMode only — Stage G-V1, the tilted point-label fixture.
// NOT registered in Tools/core-tests/core-tests.csproj (engine-bound: drives a real MapViewComponent
// through the geojson→symbol pipeline and a live LabelPlacementSystem).
//
...
// proof of point-label positional correctness in this kit (plan §Deferred scope fence: text point-labels
...
// the same "project a KNOWN world point through the live camera" oracle discipline `OffLookAtLabelScene`
...
// the mechanism under test (SymbolFeatureExtractor / LabelPlacementSystem).
//
...
{
    [TestFixture]
    internal class GeoJsonPointLabelFixtureTests
    {
        ...
        // glyph's systematic ink-centroid bias the SAME vector for both labels (what the residual-agreement
        ...
        private const string LabelText  = "A";
        ...
        // The two labels' residual VECTORS (identical text ⇒ identical systematic bias) must agree within a
        // few px — the sharper clause that isolates a real per-label placement error from the shared bias,
            ...
            (double lon, double lat) a, (double lon, double lat) b, int expectedQuads)
        {
            return VisualScene.New()
                .Source("points", GeoJson.Points((a.lon, a.lat, LabelText), (b.lon, b.lat, LabelText)))
        ...
        public void Present_And_Pos_InkLandsAtEachAuthoredAnchor_AndResidualsAgree()
        {
            ...
            // ── T-Pos: the ink centroid must sit within N px of the predicted anchor, and the two labels'
        ...
        public void Distinct_TwoSeparateInkRegions_AndExactlyTwoSurvivors()
        {
            ...
            int survivors = frame.MapView.View.Labels.LastSurvivorCount;
        ...
        public void Neg_SymbolLayerPresent_EmptyPointsSource_RendersBackgroundOnly()
        {
            ...
            // check above too. LastInputLabelCount == 0 is what proves the zero-ink frame reflects a
            // genuinely EMPTY points source rather than a pipeline that produced no labels for any input.
            int inputLabelCount = frame.MapView.View.Labels.LastInputLabelCount;
            Assert.AreEqual(0, inputLabelCount,
                $"the empty points source must feed exactly 0 labels into placement (got {inputLabelCount})");
        }
        ...
        [Test]
        public void Golden_Gv1Label_MatchesBakedReference()
        {
            AssertMidTileFence();
