// PlayMode half of the pump-split: these [UnityTest] yield-wait for genuinely off-main symbol work
// (extract/tail on the ThreadPool) — a pre-existing EditMode flake, since a yielded frame is instantaneous
// in EditMode and starves the ThreadPool. Real PlayMode frames give it wall-clock, making the wait
// deterministic. The GC.Alloc-measuring tests (PlayMode adds per-frame engine allocations to the measured
// region) and the one synchronous unit test stayed in the EditMode half — see
// MapRenderer.Tests.Text.SymbolLabelSubsystemPumpTests.
...
namespace MapRenderer.Tests.PlayMode.Text
{
    ...
    public class SymbolLabelSubsystemPumpTests
    {
        ...
        //    to the thread pool then back to main, so its labels commit only across pumped frames. ──
        [UnityTest]
