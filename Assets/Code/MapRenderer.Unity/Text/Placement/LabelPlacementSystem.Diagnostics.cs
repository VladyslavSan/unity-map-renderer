// Namespace-collision guard (see LabelPlacementSystem.cs's header): this partial lives in
...
// questions about a heavy per-frame label load ("input label count 82791, 40 ms LabelTick"): WHAT are all
// those labels (per style layer), and WHERE are they on screen (10 vertical bands, top→bottom — the
...
{
    internal sealed partial class LabelPlacementSystem
    {
        ...
        /// <summary>Set by <see cref="RequestLabelBreakdown"/>, consumed (and cleared) by the next real
        /// placement Tick's <see cref="CaptureLabelBreakdown"/>. A held (throttled) frame never runs TickCore,
        /// so the request survives until an actual recompute frame captures it.</summary>
        private bool _breakdownRequested;

        /// <summary>Arm a one-shot label-breakdown capture: the next placement Tick logs a per-layer +
        /// per-screen-band tally of this frame's input records to the Console. Idempotent — re-arming before
        /// the capture runs is a no-op. Called from the dev overlay button (<c>LabelBreakdownOverlay</c>).</summary>
        internal void RequestLabelBreakdown() => _breakdownRequested = true;
        ...
        private void CaptureLabelBreakdown(IReadOnlyList<SymbolRenderLayer> symbolLayers, double2 viewportLogicalPx)
        {
            ...
            {
                if (_mirrorRecordDropped[r] != 0) continue; // Dropped: masked out of placement, not an "input" label
                input++;

                int detail = _mirrorDetail[r];
                int slot = _mirrorKinds[r] == (byte)LabelRecordKind.Point
