namespace MapRenderer.Unity.Rendering.ShaderProperties
{
    ...
    /// (and the base URP Lit surface), in both shaders' <c>Properties{}</c> blocks and/or the shared
    /// <c>UnityPerMaterial</c> CBUFFER. Use <see cref="PropertyId"/> for <c>Material</c> calls, and this
    /// class only where Unity requires a string. Layer-specific names live in
    /// <see cref="Line.PropertyNames"/> and <see cref="Fill.PropertyNames"/>.
    /// </summary>
    public static class PropertyNames
    {
        // region: CBUFFER (UnityPerMaterial) — shared instanced members, in BOTH Line_LitInput.hlsl and
        // Fill_LitInput.hlsl CBUFFERs (SRP _*_ST / _*_TexelSize companions stripped). 14 entries.
        ...
        // Set via SetFloat with the int id. 9 entries.
        ...
        // For inspector display and keyword derivation. 18 entries.
