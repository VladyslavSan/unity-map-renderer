...
namespace MapRenderer.Unity.Rendering.Backend.BRG
{
    ...
    internal sealed class TileRenderer : VerifiedDisposable, ITileRenderBackend
    {
        ...
        // slot emits nothing. Absent or short ⇒ visible, identically in all three backends.
        ...
        internal int ComputeEmitOrder(List<int> dst, BatchCullingViewType viewType)
        {
            ...
            for (int i = 0; i < _sortedItems.Count; i++)
            {
                ...
                if (GatedOut(item.MaterialIndex)) continue;
        ...
        /// <summary>True when <paramref name="materialIndex"/>'s slot is gated out, so it emits no draw
        /// command in ANY view. One list read per item per cull — see <see cref="ComputeEmitOrder"/>.</summary>
        /// <param name="materialIndex">The layer's global SLOT.</param>
        ...
        public void SetLayerVisible(int slot, bool visible)
        {
            ...
            if (_layerGatedOut[slot] == !visible) return; // unchanged ⇒ nothing to update
