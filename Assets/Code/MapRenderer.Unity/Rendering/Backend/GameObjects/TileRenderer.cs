...
namespace MapRenderer.Unity.Rendering.Backend.GameObjects
{
    ...
    internal sealed class TileRenderer : VerifiedDisposable, ITileRenderBackend
    {
        ...
        // slot's children are disabled. Absent or short ⇒ visible, identically in all three backends.
        ...
        public void SetLayerVisible(int slot, bool visible)
        {
            ...
            if (_layerGatedOut[slot] == !visible) return; // unchanged ⇒ no walk over the items
        ...
        public int AddTileLayer(Mesh mesh, double3 tileOriginRender, int materialIndex, TileId tileId)
        {
            ...
            // Not unconditionally true: MeshNode builds and releases DISABLED, and an item added into an
            // already-gated slot must stay that way until the gate lifts.
            node.Renderer.enabled        = !GatedOut(materialIndex);
