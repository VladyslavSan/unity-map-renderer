...
namespace MapRenderer.Unity.Rendering.Style
{
    ...
    internal static class SurvivingLayerGate
    {
        ...
        /// <c>text-halo-width</c>/<c>-blur</c> satisfy both halves too and are deliberately withheld, since
        /// no shipped fixture differs on them (widen with a real RED case, not "for consistency").
        /// Every key here is free at any non-data-driven kind (<see cref="IsFreeAtKind"/>'s default arm)
        /// EXCEPT <c>text-color</c>, whose own arm defers to
        ...
        /// explicit and non-fallback), non-data-driven on BOTH sides, and — <c>text-color</c> only — equal
        /// in ALPHA on both sides (see <see cref="ConstantAlphaMatches"/>).
        ///
        ...
        private static HashSet<string> FreelyTransitionableKeys(StyleLayer oldLayer, StyleLayer newLayer)
        {
            ...
            foreach (string key in TransitionablePaintKeys)
            {
                ...
                if (key == SymbolProperty.TextColor && !ConstantAlphaMatches(oldValue, newValue)) continue;
                free.Add(key);
            }
            return free;
        }
        ...
        /// <summary>
        /// True iff two Constant <c>text-color</c> expressions evaluate to the SAME alpha. <c>text-color</c>'s
        /// RGB rides the <c>_TextColor</c> uniform (<see cref="SymbolTextColorCarrier"/>), but its alpha
        /// travels by the vertex COLOR stream instead (<c>SymbolFeatureExtractor.EvaluatePaint</c>'s
        /// <c>textRgba.w</c>) — a carrier the in-place path never re-bakes, so an alpha-only change must
        /// still refuse. Both values are already proven Constant by <see cref="IsFreeAtKind"/>, so
        /// <c>Evaluate</c> cannot throw.
        /// </summary>
        ...
        /// <c>text-color</c> defers to <see cref="SymbolTextColorCarrier.RidesUniform(ExpressionKind)"/>,
        /// since at any other kind it bakes into the vertex COLOR stream instead, which the in-place path
        /// never re-bakes (<c>MapView</c>'s <c>SymbolSubsystem.SetStyle</c> skip). A parse failure is not
        /// free (fail closed).
        /// </summary>
        private static bool IsFreeAtKind(string key, JsonValue expr)
        {
            ...
            return key == SymbolProperty.TextColor
