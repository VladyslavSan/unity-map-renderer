...
namespace MapRenderer.Unity.Rendering.Tile
{
    ...
    /// <c>JsonCanonical.Digest</c>) — the numbering component matters because a <c>MapMaterialSet</c> field
    /// Build reads is live-mutable and not part of the style document, so it can shift dense layer ids under
    /// unchanged content. <c>JsonCanonical.Digest</c> is a 64-bit FNV-1a hash, not a cryptographic one — two
