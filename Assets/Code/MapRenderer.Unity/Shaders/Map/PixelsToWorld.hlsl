// PixelsToWorld.hlsl — shared map helper: MapPixelsToWorld.
//
...
//
// Fill_VertexModify.hlsl, Line_VertexExtrude.hlsl and FillExtrusion_VertexModify.hlsl all include
// this ONE copy, so the three layers cannot drift apart and no sentinel test keeps copies in step.
//
...
// metres-per-pixel scalar (such as a _MetersPerPixel uniform) cannot express either.
//
// ── The w-ratio is CORRECTNESS, not a refinement. Derivation, because it is not obvious ────────────
...
//
// Uncorrected, the helper would return the true scale times wRef/w0 = 1 + e, with
// e = 0.02*tan(fov/2)*dot(dirWS,fwd), while -dirWS would return it times (1 - e): TWO DIFFERENT RULERS FOR
// ONE PHYSICAL AXIS, 1.910% apart at fov 60 / tilt 55, 2.336% at the fov-60 ceiling, exactly 0 when dirWS is
// perpendicular to the view axis. The ribbon's two vertices at a station carry opposite extrudeN and hit
// exactly that. What it would cost:
//   - WIDTH: the band's two edges would be sized with different rulers. Their WORLD separation stays
//     2*halfWidth — the errors cancel before the perspective divide — but the band's centre would sit
//     e*halfWidth off the centreline, and because screen position is rational in world offset the RENDERED
//     width would move too: 0.008 px on a 16 px band, 0.48 px on a 120 px one.
//   - LINE-OFFSET: also paired, but the two vertices take a COMMON offset each with its own ruler, so an
//     offset of L pixels would leak e*L into the HALF-WIDTH — 12.1% at L = 160 on a 24 px line, and
//     unbounded in L, because nothing bounds it by the styled width.
...
// Two properties follow: the result does not depend on the 0.02 probe length at all (it cancels), so that
// constant is a floating-point PRECISION choice and not an accuracy one; and under an orthographic
// projection wRef == w0 bitwise, so the factor is 1.0 and the correction changes no bit of the result.
//
// ORDER MATTERS: the correction multiplies the PIXEL SPAN, before the clamp. refPx then means what it
