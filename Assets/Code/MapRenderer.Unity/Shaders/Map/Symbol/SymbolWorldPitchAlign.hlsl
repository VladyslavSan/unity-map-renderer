// SymbolWorldPitchAlign.hlsl — W2: the map-pitched symbol vertex position.
//
...
//
//   ⚠ THE PROHIBITION. Three previously-committed stages (P3a/P3b/P3c, reverted at f05c0595) all died by
//   re-deriving a px→metre ruler inside this shader. Nothing in this file may read
//   `_MapFrameMetersPerDevicePixel`, read `UNITY_MATRIX_P._m11`, read `clip.w` as a size ruler, or introduce
//   any global/uniform converting pixels to metres. If a future edit here seems to need "how many metres is
//   a pixel at this vertex", that edit has re-derived a reverted design. It hasn't; the offsets are already
//   metres and the perspective divide is the entire ruler.
//
...
// in a y-DOWN frame (the A0-F2 negation in BillboardMath.BuildWorldQuad), and BillboardMath's own doc states
// outright that this convention must NOT be re-derived on paper, because the previous paper reading was
// self-contradictory. So which of ±cross(upWS, x̂) is "downward on screen" was established the way P3b
// established its analogue: BOTH values were gated on ONE tree, and the one whose tilt-0 render matches the
// VIEWPORT arm was kept. The viewport arm is the reference precisely because it shares no code with this
// branch — SymbolWorldIsMapPitched sends the two down mutually exclusive paths.
//
...
//          The count clause is blind to it BY CONSTRUCTION — a mirror is an isometry.
...
//
// WORLD space, not object space, deliberately: TransformObjectToWorldDir normalizes, so the frame carries no
// dependence on the object transform's scale, and render space is metres by the same definition W1 already
// relies on (PolylineArcMath.BuildCumulativeWorld's `total` is metres in this space).
bool SymbolWorldGroundFrame(float3 tangentOS, float3 upOS, out float3 xh, out float3 yh)
{
    ...
    //
    // NOTE FOR THE RECORD (followUp F-W2-3, inherited verbatim from P3b's F1): this projection is INERT on
    // every Mercator fixture in the suite — upWS is the constant (0,1,0) and every baked road tangent is
    // horizontal, so `axial == 0` exactly and `xh == tWS`. Its stated purpose only exists on a globe
    // (spherical is this renderer's default projection). No tooth in W2 discriminates the operand order
    // here; closing that needs a spherical curved-map fixture. Do not claim otherwise.
...
float4 SymbolWorldMapPitchClip(float3 anchorOS, float3 tangentOS, float3 upOS, float2 offsetMetres)
{
    ...
    if (!SymbolWorldGroundFrame(tangentOS, upOS, xh, yh))
    {
        ...
        //
        //   ⚠ DO NOT "simplify" ŷ to ±UNITY_MATRIX_V[1].xyz. That was written here first, from the guess that
        //   cross(V[2], V[0]) == V[1], and it is WRONG: Unity's world basis is left-handed under the standard
        //   cross product (right × up == forward), so for (V[0], V[1], V[2]) == (right, up, −forward) the
        //   product is cross(V[2], V[0]) == −V[1]. The error was invisible to a paper reading and was caught
        //   by MEASUREMENT — at tilt 0 it gave the fallback the OPPOSITE screen sense to the ground frame, so
        //   W2-T9 and W2-T4b disagreed about the sign and exactly one of them could ever be green. Writing
        //   the cross product out keeps the two branches provably continuous (at tilt 0 the surface normal IS
        //   V[2].xyz, so the two expressions are then the same number) and needs no handedness claim at all.
