...
```
  LineLayerInput { Geometry, FeatureSelected, OriginRender, Projection,
                   Join, Cap, MiterLimit, RoundLimit, RoundSegments, MaxOutputVertices }
        │
        │  LineRingGatherJob        this layer's own selection + LineString-kind + length(≥2) gate,
   ...
   (original, projected)
        │
        │  LineSubdivideJob         curvature-subdivides in TILE SPACE; a flat projection's ∞
  ...
  PROJECT branch                                  SIZE branch
        │                                              │
        ▼                                              ▼
   TileToGeoJob                              LineRibbonSizingJob
  ...
  (subdivided, projected)                            │
        ...
        └──────────────────────┬───────────────────────┘
                                ▼
                         LineRibbonBatchJob   IJobParallelForDefer, one ring per Execute
                                ▼             call; runs LineRibbonJob's join/cap/topology,
                            ribboned          RAW and unswapped per ring
        │
        │  LineRibbonAggregateJob   walks rings IN RING ORDER: winding swap + running-offset
...
falls out of `LineRibbonJob`'s own join/cap decisions — exactly the earcut's reason for the same three-stage
shape in `Fill/README.md`. `LineRibbonBatchJob` writes each ring's output at its own offset, ring-local and
unswapped; `LineRibbonAggregateJob` is where those become the graph's real, globally-offset, winding-correct
...
| `LineLayerInput` | the input descriptor — the line twin of `FillMeshPipeline.LayerInput` |
...
| `LineRingGatherJob` | selection + kind + length gate, gathering this layer's LineString rings off the borrowed shared geometry |
| `LineSubdivideJob` | curvature-subdivision in tile space, over the per-point surface `up` |
| `LineRibbonSizingJob` | per-ring offset tables; the capacity guard |
| `LineRibbonBatchJob` | the parallel, per-ring ribbon build (`IJobParallelForDefer`, deferred over the sizing job's per-ring vertex count) |
| `LineRibbonJob` | the 3D ribbon builder itself — one ring's join/cap geometry and index topology |
| `LineRibbonAggregateJob` | winding swap, offset rebase and the overflow stop, applied in ring order |
| `LineRibbonBuffers` | the flat working columns the sizing/ribbon/aggregate stages share |
...
- **The caller owns the inputs** (`LineLayerInput.Geometry`, `.FeatureSelected`) and must not dispose them
...
- **Errors are a flag, not a throw.** `LineRibbonSizingJob` and `LineRibbonAggregateJob` share one
