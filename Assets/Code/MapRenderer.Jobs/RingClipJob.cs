...
namespace MapRenderer.Jobs
{
    ...
    {
        /// <summary>Scratch sizing, as a multiple of the LONGEST input ring — <c>ScratchA</c>/<c>ScratchB</c>
        ...
        public const int ScratchLengthMultiplier = 16;
        ...
        // ── Scratch (ping-pong, one ring at a time) ────────────────────────────────────────────
        // Sized ScratchLengthMultiplier × the longest input ring; never read across rings.
        public NativeArray<double2> ScratchA;
        public NativeArray<double2> ScratchB;
        ...
        /// buffers. Returns the surviving vertex count; the survivors always end up in <see cref="ScratchA"/>
        ...
        /// <summary>
        /// One Sutherland–Hodgman pass: reads <paramref name="len"/> vertices from <see cref="ScratchA"/>,
        /// writes the survivors to <see cref="ScratchB"/>, then swaps so the result is back in
        /// <see cref="ScratchA"/>. Returns the new length.
        /// </summary>
        private int ClipAgainstPlane(int len, int axis, double boundary, bool keepAbove)
        {
            int outLen = 0;
            double2 prev = ScratchA[len - 1];
            bool prevInside = Inside(prev, axis, boundary, keepAbove);

            for (int i = 0; i < len; i++)
            {
                double2 cur = ScratchA[i];
            ...
            if (outLen > 1 && ScratchB[outLen - 1].Equals(ScratchB[0]))
                outLen--;

            NativeArray<double2> consumed = ScratchA;
            ScratchA = ScratchB;
            ScratchB = consumed;
            return outLen;
        }
        ...
        {
            if (outLen > 0 && ScratchB[outLen - 1].Equals(v)) return outLen;
            ScratchB[outLen] = v;
            return outLen + 1;
        }
