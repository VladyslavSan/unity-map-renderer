...
{
    /// <summary>
    /// Camera-preset page: ~10 slots to Save the live camera pose and Load it back later — the reproducer
    /// for "where did I see that state?". Select a slot, then Save (store the current view) / Load (jump the
    /// camera to the slot) / Clear. Slots persist through
